...
- 예시 코드
    ...
    ```java
    public static long time(Executor executor, int cncurrency, Runnable action) throws InterrupedException {
...
- 주석 기재 위치
    ...
    - 반환 타입만으로 아 수 없는 정적 팩터리 메서드의 경우에도 반환 타입의 안정성 기재
- 외부 사용 가능한 락 제공하기
    - 재공 시 클라이언트에서 원자적 작업 호출 구현이 가능
