...
- 스트림의 핵심 개념
  ...
  - 스트림 파이프라인(stream pipeline): (스트림) 원소들로 수향하는 연산 단계를 포현
- 스트림의 원소가 될 수 있는 것들
...
- 스트림과 반복 중 무엇을 사용할 지 명확하지 않은 경우
  ...
  - Cartesian Product의 예시
    ...
    - 스트림을 통한 구현
      ...
      - `flatMap`: 스트림의 원소 각각을 하나의 스트림을 ㅗ매핑한 이후에, 그 스트림을 하나의 스트림으로 합침(평탄화 flatening)
...
- 스트림 답지 않은 스트림 API 사용 코드
  ...
  - **`forEach`는 그저 스트림이 수행한 연산 결과를 보여주는데만 사용하라! 계산에 사용하지 말자!**
    ...
    - 가끔 계산 결과르 기존 컬렉션에 추가하는 등의 용도로 사용할 수는 있다.
...
- 자바의 동시성 프로그래밍
  ...
  - 자바5: `java.util.concurrent` 라이브버리, 실행자(`Executor`) 프레임워크 지원
