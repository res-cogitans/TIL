...
- `java.util.function`: 다양한 표준 함수형 인터페이스 제공
  ...
  - 자료형에 따른 변형 존재
    ...
    - `Function`의 경우
      - 반환 타입만 매개변수화하여 받는 형태의 변형 존재
        - `LongFunction<int[]>`: 인수는 `Long` 반환 타입은 `int[]`
      - 입력과 결과 타입이 모두 기본 타입: `SrcToResult` 형태
        - `LongToIntFuction`
      - 입력이 객체 참조이고, 결과가 기본형인 변형: `ToResultFunction<T>`
        - `ToLongFunction<int[]>`
  - 인수 수에 따른 변형 존재
    - 인수를 두 개씩 받음: `Bi-`
      - `BiPredicate<T, U>`, `BiFunction<T, U, R>`, `BiConsumer<T, U>`
      - `BiFunction`은 기본타입 반환 변형이 여기 추가로 존재: `ToIntBiFunction<T, U>` 등

- 표준 함수형 인터페이스 대부분은 기본 타입만 지원
  ***하지만, 기본 함수형 인터페이스에 박싱된 기본 타입을 넣어 사용해선 안 된다!(성능 문제)***

- 표준 함수형 인터페이스 대신, **새로 함수형 인터페이스를 구현해야 하는 경우**

  - 사용하려는 인터페이스가 표준 함수형 인터페이스에 없을 때
  - 자주 사용되며, 이름 자체가 용도를 명확히 설명
  - 반드시 따라야 하는 규약이 있음
  - 유용한 디폴트 메서드 제공 가능

- 함수형 인터페이스 사용 시 주의사항

  - `@FunctionalInterface` 애너테이션을 명시하여 의도를 확실히 함
  - 함수형 인터페이스를 같은 위치의 인수로 받는 메서드를 오버로딩 해선 안 된다!
    - 함수형 인터페이스 `Callable<T>`와 `Runnable`이 있다고 할 때,
      `submit(Callable<T> toCall)`, `submit(Runable toRun)`은 혼란을 줌



## 아이템45. 스트림은 주의해서 사용하라

- 스트림의 목적: 대량 데이터 처리 작업을 위함
- 스트림의 핵심 개념
  - 스트림(stream): 데이터 원소의 유 / 무한 시퀀스(sequence)
  - 스트림 파이프라인(stream pipeline): (스트림) 원소들로 수행하는 연산 단계를 포현
- 스트림의 원소가 될 수 있는 것들
  - 컬렉션, 배열, 파일, 정규표현식 패턴 매처, 난수 생성기, 다른 스트림 등
  - 객체 참조 혹은 기본 타입(`int`, `long`, `double`)
- 스트림의 연산 흐름
  - 소스 스트림
  - 중간 연산(intermediate operation): 한 스트림을 다른 스트림으로 변환
  - 종단 연산(terminal operation): 최종적 연산
    - 원소를 정렬해 컬렉션에 담음
    - 특정 원소 하나를 선택
    - 모든 원소를 출력
- 스트림 파이프라인은 지연 평가(lazy evaluation)
  ...
  - 종단 연산이 있기에 무한 스트림을 다룰 수 있음
  - 종단 연산이 없다면 스트림은 아무 일도 하지 않음
- 스트림은 fluent API
- 기본적으로 스트림은 순차적으로 실행됨
  - 파이프라인 병렬 실행은 `parallel` 메서드로 가능하지만, 효과를 볼 수 있는 상황은 적음
- `computeIfAbsent()`
  - 맵 안에 키가 있다면, 그 키에 매핑된 값을 반환
  - 키가 없을 경우 전달된 함수 객체를 키에 적용하여 값을 계산
...
- ***스트림을 과다하게 사용할 경우 프로그램을 읽거나 유지보수하기 어려워진다.***
- 람다에서는 매개변수의 타입을 종종 생략하기에 매개변수 명명에 주의를 기울여야 한다.
  - 도우미 메서드의 활용은 스트림 파이프라인에서 더 중요하다. 
    람다와 마찬가지로 타입을 종종 생략하며 임시 변수 사용이 잦기 때문이다.

- `char` 등 자료형을 취급할 때는 스트림을 안 쓰는게 낫다.
- 반복문을 스트림으로 리팩토링하는 것이 안 좋은 경우도 있다. 리팩토링 후 결과물이 더 나을 때만 수정하자.
- 함수 객체로는 수행할 수 없고, 코드 블록으로 수행할 수 있는 일들: ***스트림과 맞지 않는 일임!***
  - 범위 내의 지역 변수 읽기 / 수정
    - 람다의 경우 `final`이거나 사실상 `final`인 변수만 읽을 수 있음, 수정은 사실상 불가
  - `return`, `break`, `continue`, 예외를 이용한 제어0
- 스트림이 적합한 경우
  - 원소들의 시퀀스를 
    - 일관되게 변환,
    - 필터링, 
    - 연산을 이용한 결합(합 구하기 등), 
    - 컬렉션에 모음, 
  - 원소 시퀀스 내 특정 조건의 원소 탐색
- 스트림을 사용하기 어려운 경우

  - 한 데이터가 파이프라인의 여러 단계를 통과할 때 데이터의 각 단계에서의 값들에 동시에 접근하기 어려운 경우

- 스트림을 반환하는 메서드명은 복수 명사로 작성해주자
- 스트림과 반복 중 무엇을 사용할 지 명확하지 않은 경우

  - Cartesian Product의 예시
      ...
      private static List<Card> newDeck() {
          return Stream.of(Suit.values())
              .flatMap(suit -> 
                      ...
                      .map(map(rank -> new Card(suit, rank)))
                      .collect(toList()));
      }
      ```

      - `flatMap`: 스트림의 원소 각각을 하나의 스트림으로 매핑한 이후에, 그 스트림을 하나의 스트림으로 합침(평탄화 flatening)

    - 스트림 방식이 괜찮아 보이고, 동료들도 코드를 이해할 수 있으며 선호할 때 스트림 방식을 도입하자.
...
- 스트림을 제대로 이해하고 잘 사용하기 위해서는 함수형 프로그래밍에 대한 이해가 필요하다.

- 스트림의 각 변환 단계는 ***순수 함수가 되어야 한다.***

  - 오직 입력만이 결과에 영향을 줘야 한다.
  - 다른 가변 상태를 참조하지 않고, 함수 스스로도 다른 상태를 변경해선 안 된다.
  - ***이를 위해 순수 함수여야 한다.***

- 스트림 답지 않은 스트림 API 사용 코드
  ...
  - `forEach`에서 외부 상태(빈도표)를 수정한다.

  - **`forEach`는 그저 스트림이 수행한 연산 결과를 보여주는데만 사용하라! 계산에 사용하지 말자!**

    - 가끔 계산 결과를 기존 컬렉션에 추가하는 등의 용도로 사용할 수는 있다.
  ...
  - 스트림답게 수정한 코드
    ...
    - `collector`
      ...
      - `toMap(keyMapper, valueMapper)`
        ...
        - 스트림의 각 원소가 스트림의 각 원소가 고유한 키에 매핑되어 있을 떄 적합

        - 여러 원소가 같은 키를 사용할 경우 `IllegalStateException` 방생

          - `merge()` 함수를 제공하거나 `groupingBy()` 사용하여 충돌을 피할 수 있음

        - 특정 키의 원소 중 하나를 골라 연관짓는 경우 유용
      ...
      - `groupingBy()`
        ...
        - 입력으로 분류 함수(classfier), 출력으로 카테고리별로 모은 수집기를 반환
        - 분류 함수는 카테고리를 반환하며, 해당 원소의 맵 키로 사용
        - 리스트로 모으지 않고 다른 방식으로 모으려고 하면 다운스트림 수집기도 명시해야 함
          - `toSet()`, `toCollection(collectionFactory)`, `counting()`을 넘기는 방식 등이 있음

      - `partitioningBy()`

        - 분류 함수로 predicate를 받고 키가 `Boolean`인 맵을 반환



## 아이템47. 반환 타입으로는 스트림보다 컬렉션이 낫다

- 자바8 이전의 원소 시퀀스 반환 타입

  - 컬렉션 인터페이스: 가장 기본
  - `Iterable`: for-each문 에서 사용되거나 원소 시퀀스가 일부 컬렉션 메서드(`contains()`)를 구현할 수 없는 경우
  - 배열: 기본 타입이 반환되거나 성능에 민감한 상황일 경우

- 반환 타입이 스트림일 경우 반복을 지원하지 않음

  - 스트림은 `Iterable`을 확장하지 않기에 생기는 문제
...
- `Iterable`을 반환할 경우 스트림을 지원하지 않음
...
- **원소 시퀀스를 반환하는 공개 API의 경우 `Collection`이나 그 하위 타입을 반환하자**

  - 반복과 스트림을 모두 지원하기 때문이다.
  - 하지만 덩치 큰 시퀀스를 메모리에 올려선 안 된다.
  - 반환할 시퀀스가 크지만 표현을 간결하게 할 수 있다면 전용 컬렉션을 구현하는 것을 검토하자
    - `AbstractCollection`, `AbstractList` 등을 사용하여 구현하자
  - 구현이 불가능할 경우 스트림이나 `Iterable`을 제공하고, 메서드에 따라 두 방식을 모두 제공할 수도 있다.
...
- 자바의 동시성 프로그래밍
  - 첫 릴리즈(1996): 스레드, 동기화, `wait`, `notify` 지원
  - 자바5: `java.util.concurrent` 라이브러리, 실행자(`Executor`) 프레임워크 지원
  - 자바7: 고성능 병렬 분해(parallel decompositon) 프레임워크인 포크-조인(fork-join) 패키지 추가
  - 자바8: `parallel()`메서드로 병렬 실행 가능한 스트림 지원
- 동시성 프로그래밍에서는 안정성(safety)과 응답 가능(liveness) 상태를 유지하기 위해 애써야 함



### 스트림 병렬 처리 시 주의사항

- `parallel`을 호출하는 것 만으로 병렬화가 제대로 이루어지진 않음

- **데이터소스가 `Stream.iterate`이거나 중간 연산으로`limit`를 사용할 경우 병렬화를 통한 성능 개선을 기대할 수 없다.**

- **스트림 소스가 `ArrayList`, `HashMap`, `HashSet`, `ConcurrentHashMap`의 인스턴스이거나 배열, `int`범위, `long` 범위일 때 병렬화 효과가 가장 좋은 편이다.**

  - 데이터를 원하는 크기로 쉽게 나눌 수 있기에 일을 스레드에 분배하기 용이하다.
    - 분배 작업은 `Spliterator`가 담당한다. (`Stream`이나 `Iterator`의 `spliterator()`로 얻어올 수 있다.)
  - 원소들을 순차적으로 실행할 때의 참조 지역성(locality of reference)이 뛰어나다.
    - 즉 이웃 원소가 메모리에 연속적으로 저장되어 있다.
    - 참조 지역성이 나쁠 경우 스레드는 데이터가 주 메모리에서 캐시 메모리로 전송되어 오기를 기다리게 된다.

- 종단 연산의 동작 방식도 병렬 수행 효율에 영향을 준다.

  - 스트림 연산의 작업에서 큰 비중을 차지하며, **특히 순차 연산의 경우 병렬 수행 효과를 많이 제한한다.**
  - 축소(reduction)이 병렬화에 가장 적합하다.
  - 조건에 맞으면 바로 반환하는 메서드(`anyMatch()`, `allMatch()`, `noneMatch()`)도 병렬화에 적합하다.
  - `collect()` 메서드의 경우 컬렉션을 합치는 부담으로 인해 병렬화에 부적합하다.

- 직접 구현한 `Stream`, `Iterable`, `Collection`의 경우 병렬화 이점을 높이려면
  `spliterator` 메서드를 반드시 재정의하고 성능 체크를 수행하자.

- 스트림을 잘못 병렬화했을 때의 문제

  - **안전 실패(safety failure)**
    - 결과가 잘못되거나 오동작하는 경우
    - `mappers`, `filters`나 다른 함수 객체가 명세대로 동작하지 않을 경우 발생할 수 있음
      - ***`Stream` 명세에 맞게 구현해야 함!***

- 병렬화와 성능

  - 스트림 병렬화를 적절하게 수행했다고 해도 병렬화에 드는 추가 비용을 상쇄할만큼의 이익을 얻지 못할 수도 있음
  - 스트림 내 원소 수 * 원소 당 수행되는 코드 라인 수가 최소 수십만은 되어야 성능 향상을 이룰 수 있다.
  - 병렬화는 오직 성능 최적화 수단이기에 반드시 성능 테스트를 거쳐야 한다.

  - 병렬 스트림 파이프라인도 동일 스레드풀을 사용하기에 잘못된 파이프라인은 시스템의 다른 부분에도 악영향을 끼칠 수 있다.
  - **스트림 병렬화가 효과를 보는 경우는 많지 않다.**
  - 단, 조건이 잘 갖춰질 경우 병렬화 처리만으로 프로세서 코어 수 만큼의 성능 향상을 기대할 수 있다.
    - 머신러닝이나 데이터 처리 등의 분야에서 특히 강조된다.

- 무작위 수들로 이뤄진 스트림을 병렬화할 경우

  - `ThreadLocalRandom` / `Random` 보다는 `SplittableRandom`을 이용하자
    - 병렬화를 위해 설계된 것이다.
...
- 빠른 오류 감지 및 쉬운 오류 발생 지점 탐색을 위한 원칙
  - 이를 위해 생성자 / 메서드 매개변수의 제약사항을 문서화하고, 빠르게 검증해야 한다.
  - ex) 인덱스 값은 음수여선 안 된다, 객체 참조는 `null`이 아니어야 한다.
- 매개변수 검사를 제대로 하지 않았을 때 생길 수 있는 문제
  - 메서드 수행 중에 모호한 애매 발생
  - 메서드가 잘 수행되지만 잘못된 결과 반환
  - 메서드가 잘 수행되지만 객체에 부작용 발생하여 이후 다른 메서드에서 문제 발생
  - ***매개변수 검사에 실패하면 실패 원자성(failure atomicity)을 어길 수 있음***
    - 실패 원자성: 호된 메서드가 실패하더라도 해 객체는 메서드 호출 전 상태를 유지해야 함 

### 예외 문서화

예외 문서화

- `public`, `protected` 메서드의 경우 매개변수 값이 잘못됐을 때 던지는 예외를 문서화해야 한다. `@throws` 자바독 태그를 이용하면 된다.

- 매개변수의 제약을 문서화할 경우, 제약을 어겼을 때 발생하는 예외를 함께 기술하라.
  ...
  - 클래스 수준 주석으로 설명 중복을 줄일 수 있다.
    - 예를 들어, 모든 메서드의 매개변수에 대해서 `null`이 전달될 경우 NPE를 던진다면, 클래스에 공통으로 기술할 수 있다.
    - `@Nullable` 등의 애노테이션을 통해 간략한 기술이 가능은 하지만 표준적인 방법은 아니다.



### 매개변수 검증 방법

- `null` 검사의 경우 `java.util.Objects.requireNonNull()` 사용하라
...
- 범위 검사용 `Objects` 메서드들
  ...
  - 약점
    - 예외 메시지 지정 불가
    - 리스트와 배열 전용으로 설계됨
    - 닫힌 범위(closed range; 양 끝단 값을 포함하는 범위)는 다루지 못함

- 비공개 메서드의 검증

  - 내가 패키지 제작자이기에, 메서드 호출 상황 자체를 통제할 수 있음

  - 단언문(`assert`)을 이용하여 검증
    ...
    - **단언문은 해당 조건이 무조건 참임을 선언하는 것**
    - **단언문과 일반적인 유효성 검사의 차이**
      - 실패하면  `AssertionError`를 던짐
      - 런타임에 어떤 효과도, 성능 저하도 없음
        - 자바 실행 명령어로 `-ea`나 `--enableassertions`를 선언하면 영향을 주게 만들 수 있음



### 주의사항

- 메서드가 직접 사용하지 않지만, 나중에 사용하기 위해 저장하는 매개변수는 철저히 검사하라.
  - ex) `int[]`의 `List<Integer>` 뷰를 반환하는 메서드
  - 만약 메서드에서 저장하는 시점에서 `null` 검사를 수행하지 않는다면, 나중에 반환된 뷰를 사용할 때 NPE가 발생하게 된다.
  - 생성자의 경우는 이 원칙의 특수 사례다. 클래스 불변식을 어기는 객체를 만들지 않기 위함이다.
- 매개변수 검사를 하지 않는 것이 나은 경우
  - 유효성 검사 비용이 지나치게 높거나 실용적이지 않을 때
  - 계산 과정에서 암묵적으로 검사가 수행될 경우
    - ex) `Colections.sort()`의 경우 정렬 과정에서 비교작업이 발생하는데, 어차피 비교 과정에서 비교 불가능한 객체임이 드러날 경우 예외가 발생한다. (`ClassCastException`) 따라서 비교할 수 있는 객체인지 검사하는 것은 의미가 없다.
    - 하지만, **암묵적 유효성 검사에 지나치게 의존할 경우 실패 원자성이 깨질 염려가 있다.**
- 유효성 검사시에 명세와 다른 예외가 발생하는 경우 예외 번역(exception translate)을 통해 명세와 맞춘다.
- ***매개변수를 검사해야 한다는 것은, 매개변수를 제약하는 게 좋다는 의미가 아니다! 매개변수는 범용적으로 설계할 수록 좋다.***
...
- 자바는 안전한 언어
  ...
  - 네이티브 메서드를 사용하지 않기에 C, C++에서 보기 쉬운 버퍼 오버런, 배열 오버런, 와일드 포인터 등 메모리 충돌 오류에서 안전
  - 메모리 전체를 거대 배열로 다루는 언어와 달리, 자바로 작성한 클래스의 불변식은 안정적으로 지켜짐
  - 하지만, 자바를 안전하게 다루기 위해서는 **불변식을 깨트리려는 공격을 가정하고 방어적으로 프로그래밍 하려고 노력해야 한다.**



### 생성자에서의 방어적 복사를 통해 막을 수 있는 케이스
...
- `Date`가 가변이라는 점을 이용하여 `Period`의 내부 값을 변경할 수 있다.

  - `Date`가 낡은 API라 발생한 문제
  - 자바 8 이후에는 불변인 `Instant`와 `LocalDateTime`, `ZoneDateTime`을 사용하여 해결 가능하다.

- 위와 같이 불변이 아닌 객체의 내부를 보호하려면,
  **생성자에서 받은 가변 매개변수 각각을 방어적으로 복사(defensive copy)해야 한다.**
  ...
  - 수정한 생성자 코드
    ...
    - ***매개변수 유효성 검사 이전에 방어적 복사본을 만들고, 복사본을 기반으로 유효성 검사를 수행하고 있다.***
      - ***멀티 스레드 환경의 경우 원본 객체 유효성 검사 - 복사본 생성 사이의 시점에 원본 객체의 수정 위험이 있기 때문이다!***
      - TOCTOU; time-of-check / time-of-use 공격
        - 검사시점 / 사용시점 공격
    - `clone()`을 이용하여 복사하고 있지 않다.
      - `Date`는 `final`이 아니기 때문에 `Date`가 정의하지 않은 `clone()`을 사용할 수 있기 때문이다.
      - **매개변수가 제3자에 의해 확장될 수 있을 경우 방어적 복사본 생성 시에 `clone()`을 사용해선 안 된다!**



### 접근자에서의 방어적 복사를 통해 막을 수 있는 케이스
...
Date end = new Date();
Period p = new Period(start, end);
p.end.setYear(78);	//p의 내부 상태 변경
```

- 접근자 메서드가 내부의 가변 정보를 그대로 드러내기에 생기는 문제

- **접근자 메서드가 필드의 방어적 복사본을 반환하게 만들면 해결**
...
- 접근자 메서드의 경우 `clone()`을 사용하여 방어적 복사를 수행해도 된다.
  - 논의 필요한 사항
  - `clone()` 자체는 `Date.clone()`이 실행됨



### 정리

- 생성자, 접근자를 방어적 복사방식으로 바꾸면 완전한 불변 클래스가 구성됨
  - 단, 리플렉션, 네이티브 메서드 등 언어 외적 수단을 동원한다면 뚫릴 수 있음
  - 필드의 완전한 캡슐화가 목적
- **불변이 아니라도, 객체 참조를 내부에 보관할 경우, 변경 가능성을 항상 고민해야 함!**
  - 참조되는 가변 객체가 변경되었을 때 객체 동작이 비정상적으로 이뤄질 수 있다면 방어적 복사 필요
  - 길이가 1 이상인 배열도 해당함에 주의
- 방어적 복사는 성능 저하를 유발하고, 항상 사용 가능한 것도 아님
  - 같은 패키지에 속하는 등의 이유로 호출자가 내부 수정을 하지 않으리라고 확신한다면 생략 가능
  - 하지만 이 경우에도 매개변수 혹은 반환값을 수정해선 안 된다는 것을 **문서화**해야 함
  - 전달되는 가변 객체의 통제권을 넘겨주는 상황의 경우에도 방어적 복사를 하지 않아도 됨
    - 이 경우 넘겨 받은 클라이언트가 해당 객체를 직접 수정하지 않음을 약속해야 하며,
      이에 대한 문서화를 메서드 및 생성자에 기재해야 함
    - 통제권을 넘겨주는 경우 악의적 클라이언트의 공격에 취약하기에
      방어적 복사의 생략은 다음 같은 경우로 한정해야 한다.
      - 해당 클래스와 클라이언트가 상호 신뢰할 수 있을 떄
      - 불변식이 깨지더라도 영향이 클라이언트에게만 갈 때
        - ex) 래퍼 클래스
...
- **메서드 이름을 신중히 짓자.**
  - 표준 명명 규칙을 따라야 한다.
  - 이해할 수 있고, 같은 패키지와 일관되게 지어라.
  - 개발자 커뮤니티에서 널리 받아들여지는 명칭을 쓰자.
- **편의 메서드를 너무 많이 만들지 말자.**
  - 메서드가 너무 많을 경우 학습, 사용, 문서화, 테스트, 유지보수가 어렵다.
- **매개변수 목록은 짧게 유지하자.**
  - 4개 이하가 좋다.
  - 같은 타입 매개변수가 연속되는 경우를 피하자.
  - **매개변수 목록을 줄이는 기법**
    - 여러 메서드로 쪼개기
      - 잘 쪼개면 직교성(orthogonality)을 높여 메서드 수를 줄일 수 있다.
        - 직교하는 두 벡터의 내적은 0
        - 설계에 있어서 직교성이 높다는 것은 중복 없이 잘 쪼개진 기능이라고 생각하면 된다.
    - 매개변수 여럿을 묶는 도우미 클래스 만들기
      - 정적 멤버 클래스로 만드는 경우가 많음
    - 빌더 패턴을 메서드 호출에 응용하기
      - 매개변수가 많으면서, 일부는 생략해도 될 때 유용
      - 매개변수를 묶는 클래스를 이용해서 전달하되, 해당 클래스 인스턴스는 빌더를 이용해서 설정
- **매개변수 타입은 클래스보다 인터페이스가 낫다.**
  - `HashMap` 대신에 `Map`을 인자로 넘기는 것과 같은 이유다.

- **`boolean`보다는 원소 두 개짜리 enum을 매개변수로 사용하는 편이 낫다.**
  - 메서드명명상 `boolean`이 적합한 경우는 당연히 `boolean` 쓴다.
  - 일반적으로는 `enum`이 가독성이 좋으며, 확장성도 우월하다.
...
- ***오버로딩(overloading)된 메서드 중 무엇을 호출할지는 컴파일타임에 결정된다!***
  - 위의 경우 의도와 다르게 `"그 외"`만 3회 출력된다.

>  **재정의한 메서드는 동적으로 선택되고,
> 다중정의한 메서드는 정적으로 선택된다.**

- 재정의의 경우 원하는 대로 작동할 확률이 높은 반면,
  다중정의의 경우 그렇지 않기에, **다중정의가 혼동을 주는 상황 자체를 피해야 한다!**
  - 메개변수 수가 같은 다중정의는 만들지 말자.
  - 가변인수 사용 메서드의 경우 다중정의를 하지 않는 것을 기본으로 하자.
  - 다중정의 대신 메서드 이름을 다르게 지어주는 방식을 고려하자.
    - `ObjectOutputStream`의 경우 `write()`를 다중정의하는 방법 대신, `writeBoolean()`, `writeInt()` 등 각각의 이름을 지어주었다.
    - 이 경우 `readBoolean()`, `readInt()`와 메서드 이름을 짝지울 수 있다는 강점도 있다.

- **생성자 다중정의**

  - 생성자의 경우 이름을 다르게 지을 수 없기에 두 번째 생성자부터는 다중정의가 된다.

  - 하지만 정적 팩터리 메서드를 대안으로 사용할 수 있다.

  - 생성자의 경우 재정의가 불가하기에 다중정의와 재정의를 혼동할 걱정은 없다.



- 매개변수 중 하나 이상이 근본적으로 다를 경우 문제가 해소된다.

  - 여기서 근본적으로 다르다는 것은, `null`이 아닌 경우 두 타입을 형변환할 수 없음을 말한다.

  - 이 경우 호출할 다중정의 메서드의 선택은 런타임 타입으로 결정된다.
  ...
  - 오토박싱 도입으로 인한 주의사항
    ...
    - 오토박싱(Since Java5)
      ...
      - `remove(i)`가 집합의 경우 음이 아닌 값 `i`를 제거하고,
        리스트의 경우 인덱스가 `i`인 값을 제거하는 결과가 발생한다.

      - 위의 경우 `list.remove((Integer) i)`로 형변환해주면 명확하게 작동한다.

      - `List`의 경우 특정 인덱스의 값을 삭제하는 `remove(int index)`가 다중정의되어 있고,
        java5 이후에는 오토방식으로 인해 `Object`와 `int`가 근본적으로 다르지 않기 때문이다.
    ...
    - 람다와 메서드 참조의 경우
      ...
      - 2번만 컴파일 에러가 발생
      - `submit()` 다중정의 메서드 중에는 `Runnable`이 아니라 `Callable`을 갖는 시그니처도 있다.
      - **다중정의된 메서드가 함수형 인터페이스를 인수로 받을 경우,
        서로 다른 함수형 인터페이스라도 인수 위치가 같을 경우 혼란이 생긴다!**

    - `Object`가 아닌 클래스 타입과 배열은 근본적으로 다르다.

    - `Serializable`과 `Cloneable` 외의 인터페이스 타입과 배열도 근본적으로 다르다.

    - `String`과 `Throwable`처럼 상/하위 관계가 아닌 두 클래스는 관련 없다(unrelated).

      - 관련 없는 클래스들도 근본적으로 다르다.

    - 다중정의 메서드 호출이 애매하더라도 기능이 동일할 경우 문제 없다.

      - **특수한 다중정의 메서드에서 일반적인 다중정의 메서드로 foward하는 것이 대표적이다.**

    - 자바 라이브러리의 위반 사례

      - `String`의 `valueOf(char[])`과 `valueOf(Object)`
...
- 가변인수 메서드
  ...
  - 호출 시 인수의 개수와 길이가 같은 배열을 생성, 해당 배열에 인수를 저장, 메서드에 전달
  ...
  - 가변인수 수 제한을 둬야 할 경우
    ...
    - 문제점
      ...
      - 인수가 0개일 경우 **런타임에 실패하게 된다!**
      - 유효성 검사를 명시적으로 수행해야 한다.
      - `min`의 초기값을 `Integer.MAX_VALUE`로 설정하지 않는다면 `for-each` 문을 사용할 수 없다.

    - 해결: 매개변수를 두 개 받도록 만들기
...
- 가변인수의 성능 문제
  ...
  - 대안: 오버로딩 이용
    ...
    - 인수가 4개 이상인 경우에만 가변인수 메서드를 사용하게 만듦
    - `EnumSet`이 대표적 사용 예
...
- 컬렉션이나 배열 같은 컨테이너(container)가 비었을 때 `null`을 반환하지 말자.

  - `null`을 반환할 경우 방어 코드를 추가적으로 삽입해줘야 하기 때문이다.
  - `null` 대신 빈 컨테이너를 할당하는 비용은 크지 않으며, 
  ...
  - 드물게 빈 컬렉션 할당으로 인해 성능을 저하시킨다면 빈 불변 컬렉션을 반환하자.
  ...
  - 배열의 경우
    ...
    - 단, `toArray()`에 넘길 배열을 미리 할당하는 것은 오히려 성능이 떨어진다.
...
- 자바 8 이전, 메서드가 값을 반환할 수 없을 때 대응
  - 예외 던지기
    - 예외는 예외 상황에서만 사용해야 한다는 부담
    - 예외 생성 시에 Stack trace 전체를 캡쳐하는 비용
  - `null` 반환
    - `null` 처리 코드가 필요해짐
    - `null` 처리를 제대로 하지 않을 경우 예상치 못한 곳에서 NPE
- `Optional<T>`: 자바 8 이후 대안
  - 원소를 최대 1개 가질 수 있음
  - 불변임
  - `null` 대신 아무 것도 담지 않은(빈) 옵셔널을 반환함
  - 강점
    - 예외를 반환하는 방식에 비해 유연하고 사용이 용이
    - `null` 반환 방식보다 오류 가능성이 적음
  - 반환 방식
    - `Optional.empty()`: 빈 옵셔널
    - `Optional.of(value)`: 값이 든 옵셔널
      - **여기에 `null`을 넣을 경우 NPE 발생!**
    - `Optional.ofNullable(value)`: `null` 값도 넣을 수 있는 옵셔널
- **옵셔널을 반환하는 메서드에서는 절대 `null`을 반환해선 안 된다!**
- **옵셔널은 반환 값이 없을 수 있음을 API 사용자에게 알리기 위함이다.**
  - Checked Exception과 같은 맥락이다.



### 클라이언트가 옵셔널을 사용하는 방식

- `orElse("기본값")`: 옵셔널이 비어 있을 경우 기본값을 받음
- `orElseThrow(IllegalArgumentException::new)`: 옵셔널이 비어 있을 경우 예외를 던짐
  - 예외가 실제 발생하지 않는 한 예외 생성 비용이 들지 않음
- `get()`: 무조건 옵셔널이 비어 있지 않다고 가정하고 꺼냄
  - 단, 비어 있을 경우 `NoSuchElementException` 던짐
- `orElseGet(supplier<T>)`: 기본값을 설정하는 비용이 너무 클 경우, 유용
- `ifPresent()`: 옵셔널이 채워져 있는지 여부를 `boolean` 값으로 반환
  - 하지만 불필요하게 긴 코드를 작성할 위험이 크다.
- 이 외의 경우, `filter()`, `map()`, `flatMap()` 등 사용 가능



### 옵셔널과 스트림

- `Stream<Optional<T>>`을 받아 `Stream<T>`를 반환하는 식으로 활용하는 경우
...
- **컨테이너 타입은 옵셔널로 감싸선 안 된다!**
  - 컬렉션, 스트림, 배열, 옵셔널 등
  - 빈 `List<T>` 등 빈 컨테이너를 반환하는 것이 좋다.
  - 빈 컨테이너를 반환할 경우 클라이언트에 별도의 옵셔널 처리 코드를 넣을 필요가 없음

> **결과가 없을 수 있으며, 클라이언트가 이 상황을 특별하게 처리해야 할 경우 `Optional<T>`를 반환하라!**

- 그럼에도 불구하고 옵셔널이 상황에 맞는지 측정할 필요가 있다.
  - 옵셔널 자체가 객체기에 그에 따른 비용이 있으며
  - 옵셔널에서 값을 꺼내는 과정에서 메서드 호출이 발생하기 때문이다.
- **박싱된 기본 타입을 담은 옵셔널을 반환하지 말자.**
  - 박싱된 기본 타입(`Integer`, `Long`, `Double`)을 감싼 옵셔널은 값을 두 번 감쌌기에 무겁다.
  - 관련 전용 클래스를 사용하자: `OptionalInt`, `OptionalLong`, `OptionalDouble`
  - 이 외의 기본 타입의 경우 중요도가 낮기에 따로 전용 클래스가 있진 않다.
- **옵셔널을 맵의 값 / 배열의 원소 등으로 사용해선 안 된다.**
  - 쓸데없이 복잡성만 높인다. (맵의 키가 없음을 두 가지 방식 - 키가 없는 경우 / 옵셔널이 빈 경우로 표현 가능)
- **옵셔널을 인스턴스 필드의 값으로 저장하지 말자. (소수의 예외 존재)**
  - 선택적으로 존재할 수 있는 값을 옵셔널로 처리하는 것도 고려할 만하다.
    - 혹은 `getter`만 옵셔널 처리하는 방식도 고려해볼 만하다.
  - 참고) JPA의 경우 필드 타입으로 옵셔널을 사용하는 것 자체를 막고 있다.
...
- JavaDoc
  - 문서화 주석(doc comment)을 이용하여 API 문서로 변환해주는 유틸리티
  - 코드 수정 시마다 일일히 API 문서를 따로 수정해줘야 하는 불편함이 없음
- **[How to Write Doc Comments](https://www.oracle.com/technical-resources/articles/java/javadoc-tool.html)**
  - 자바독 문서화 주석 작성 규칙
  - 공식 언어 명세는 아니지만 자바 개발자라면 숙지해야 함

- **공개된 모든 클래스, 인터페이스, 메서드, 필드 선언에 문서화 주석을 달아야 한다.**

  - 직렬화 가능 클래스의 경우 직렬화 형태에 대해서도 적어야 한다.

  - 기본 생성자는 문서화 주석을 달 수 없기에, 공개 클래스에는 기본 생성자를 사용해선 안 된다.
  - 유지보수를 고려한다면 비공개 클래스, 인터페이스, 생성자, 메서드, 필드에도 덜 자세하게나마 주석을 달자



### 메서드 문서화 주석

> **메서드용 문서화 주석에는 해당 메서드와 클라이언트 사이의 규약을 기술해야 한다.**

- 상속용 클래스가 아니라면 동작 원리(how)를 기술할 필요는 없다. 무엇을 하는가(what)를 기술하라!
- 메서드 호출을 위한 **전제조건(preconditions)**을 모두 나열하라!
  - 일반적으로는 `@throws` 태그로 비검사 예외를 선언하여 암시적으로 서술함. (비검사 예외 하나당 전제조건 하나)
  - `@param` 태그를 이용하여 조건에 영향받는 매개변수에 기술하는 것도 가능하다.
- 메서드가 성공적으로 호출된 후의 **사후조건(postconditon)**도 모두 나열하라.
- **부작용(side-effect)**도 기술하라
  - 예를 들어 백그라운드 스레드를 실행하는 메서드의 경우 그 사실을 명시해야 한다.
- 엄격하게 작성하려면 다음을 모두 만족해야 한다:
  - 모든 매개변수에 `@param` 태그
  - 반환 타입이 `void`가 아닐 경우 `@return` 태그
    - 코딩 표준에 따라 생략 가능하다.
...
- `@param`과 `@return` 태그의 경우 해당 값을 설명하는 명사구를 사용한다.

  - 경우에 따라 명사구 대신 산술 표현식을 사용한다(`BigInteger`의 API 문서가 그 예시다).

- `@throws` 태그의 설명은 *if + 예외를 던지는 조건 절* 설명이 붙는다.

- `@param`, `@return`, `@throws` 태그의 설명에는 마침표를 붙이지 않는다.

- 자바독 유틸리티는 문서화 주석을 HTML로 변환하기에 **HTML 요소들이 최종 문서에 반영된다.**

- `{@code}` 태그

  - 태그로 감싼 내용을 코드용 폰트로 렌더링한다.
  - 태그로 감싼 내용 속의 HTML 태그나 자바독 요소를 무시한다. `<` 등을별도의 처리 없이 사용할 수 있게 된다.
  - 여러 줄 코드를 작성할 경우 `<pre>` 태그를 이용하자: `<pre>{@code ... 코드 ...}</pre>`
    - 이스케이프 문자 없이 코드 줄 바꿈 반영된다.
    - 단 `@`에는 이스케이프 처리해줘야 한다.

- 관례상 인스턴스 메서드의 문서화 주석의 `this`는 호출된 메서드가 자리하는 객체를 지칭한다.
...
- `@implSpec`
  ...
  - 자바8에서 도입
  - 자기사용 패턴(self-use pattern)을 문서화
  - 해당 메서드와 하위 클래스 사이의 contract를 설명,
    하위 클래스들이 해당 메서드를 상속하거나 `super` 이용하여 호출 시에 어떻게 동작하는가를 인식시켜주는 것이 목적을 갖는다.
...
- `{@literal}`
  ...
  - 위의 예시의 경우 `|r| < 1이면 기하 수열이 수렴한다.`로 변환됨
  - API 문서에 HTML 메타문자(`<`, `>`, `&` 등)을 포함시키기 위해서 사용하는 태그
  - `{@code}` 태그와 달리 코드 폰트로 렌더링하지는 않음

- 요약 설명

  - 문서화 주석의 첫 문장은 해당 요소의 요약 설명(summary description)으로 간주됨

  - 요약 설명은 대상의 기능을 **고유하게** 기술해야 한다.
    -> 한 클래스(인터페이스) 내에서 동일한 요약 설명을 가진 멤버 혹은 생성자가 있어선 안 된다.

  - 요약 설명이 끝나는 부분: `<마침표><공백><다음 문장 시작>` 형태의 `<마침표>`

    - 공백: 스페이스, 탭, 줄바꿈
    - 다음 문장 시작: 소문자가 아닌 문자를 기준으로 함
    - 마침표를 포함한 텍스트(`Mrs.` 등)의 경우 `{@literal}`로 감싸서 요약 설명이 갑자기 끝나는 일을 방지하자.

  - 메서드 / 생성자의 경우 동작을 설명하는 (주어 없는) 동사구로 작성한다.

  - 클래스 / 인터페이스 / 필드의 경우 대상을 설명하는 명사절이다.
...
- 색인 기능
  ...
  - 자바9부터 자바독으로 생성된 HTML문서에 색인이 추가되었다.
  - 클래스, 메서드, 필드 등의 API 요소의 색인은 자동 생성된다.
  - `{@index}` 태그 이용하여 중요 용어를 추가적으로 색인화할 수 있다.
...
- **제네릭 타입 / 메서드를 문서화할 때는 모든 타입 매개변수에 주석을 달아야 한다.**
- **열거 타입을 문서화할 때는 상수들에도 주석을 달아야 한다.**
  - 설명이 짧다면 주석 전체를 한 문장으로 작성해도 된다.
- **애너테이션을 문서화할 때는 멤버들에도 모두 주석을 달아야 한다.**
  - 애너테이션 타입의 요약 설명은 애너테이션을 다는 것이 어떤 의미인지를 나타내는 동사구로 한다.
  - 필드 설명은 명사구로 한다.
- **패키지를 설명하는 문서화 주석은 `package-info.java` 파일에 작성한다.**
  - (자바9부터 사용되는) 모듈 관련 설명은 `module-info.java`에 작성한다.
- **스레드 안정성과 무관하게 스레드 안전 수준을 반드시 API 문서에 포함해야 한다.**
- **직렬화 가능한 클래스의 경우 직렬화 형태를 API 문서에 기술해야 한다.**
- **자바독은 메서드 주석을 상속시킬 수 있다.**
  - 세부 사항은 <자바독 참조 지침(The Javadoc Reference Guide)>을 참고하자.
  - `{@inheritDoc}` 태그 이용하여 상위 타입의 문서화 주석 일부를 상속할 수 있다.
- **문서화 주석만으로 설명이 부족할 경우 설명 문서의 링크를 제공하자.**
...
> 지역변수의 유효 범위를 최소화하면 코드 가독성과 유지보수성이 높아지며 오류 가능성은 낮아진다.

- (C99 표준 이전) C언어의 경우 지역변수를 코드 블록의 첫 머리에 선언하는 경우가 많다.
  자바의 경우에는 문장을 선언할 수 있는 곳이면 어디서든 변수를 선언할 수 있다.

- **가장 간단하고 강력한 기법: 처음 쓰일 때 선언하기**

  - 멀어질수록 잊기 쉽고 가독성도 난해해짐
  - 특히 `var` 사용할 경우에는 더 심각: [참고 링크](https://github.com/res-cogitans/TIL/blob/main/concept/var%EC%82%AC%EC%9A%A9%EC%8B%9C%20%EC%A3%BC%EC%9D%98%EC%82%AC%ED%95%AD.md)

- **거의 모든 지역변수는 선언과 동시에 초기화해야 한다.**

  - 초기화에 필요한 정보가 부족할 경우 그 정보가 충분해질 때까지 선언을 미루자.
  - **예외 사항: `try-catch`문**
    - 변수 초기화 표현식에 예외 발생 가능성이 있는 경우, 예외 전파를 막기 위해 `try-catch`문 안에 선언
    - 해당 변수를 `try` 블록 바깥에서도 사용하고 싶다면 `try` 블록 앞에서 선언

- **반복문과 변수 범위: `while` 보다는 `for`를 추천**

  - 반복 변수(loop variable)의 범위가 반복문의 body, `for` 키워드와 body 사이 괄호 안으로 제약됨

  - 이는 고전적인 `for` 문, `for-each` 형태 반복문 모두에 적용

  - ***이 측면에서, 반복문 내 변수 값을 반복문 종료 이후에도 사용하는 것이 아니라면 `while`보다 `for`문이 우월***

    - `while`문의 위와 같은 특징은, 반복 변수를 실수로 다른 반복문에 전달하게 되는 실수를 야기할 수도 있음
      ...
      while (i.hasNext()) {						//i를 다시 사용하는 실수로 버그 발생!
          doSomething(i2.next());
      }
      ```

      - 위의 경우 정상 작동으로 착각하게 되서 문제를 잡아내기도 힘듦
        -> `for`문을 쓰면 안 생길 문제

  - `for`문의 경우 `while`문보다 짧아서 가독성 면에서도 더 좋음

    ```java
    for (int i= 0, n = someExpensiveComputation(); i< n; i++) {
        //로직
    }
    ```

    - 위의 경우 반복 변수 `i`와 `n`을 짧게 선언 가능하고 범위도 최소화
    - `i`의 한계값을 `n`에 저장하여 한계값을 계산하는 비싼 로직을 여러 번 돌릴 필요를 없앴음

- **지역변수 범위를 줄이는 정석적인 방법: 메서드를 작게 유지하고 한 기능에만 집중하게 하기**

  - 메서드가 크고 여러 기능을 수행할 수록 다른 기능과 관련된 지역변수에 접근할 일이 많아지기 때문
  - 그냥 기능별로 잘 쪼개면 된다.
...
- **전통적인 `for`문의 문제점**
  - 인덱스 변수와 `Iterator` 모두 실제 순회에 필요한 데이터가 아님에도 코드를 더럽힌다.
  - 반복자, 인덱스가 코드에 더 등장할수록 오류 가능성이 증가하며, 컴파일러가 잡아 주는 문제도 아니다.
  - 컬렉션인지, 배열인지에 따라 코드 형태도 달라진다.
- **향상된 `for` 문(enhanced `for` statement)**
  - 인덱스 변수나 반복자를 사용하지 않음
  - `Iterable`을 구현하기만 하였다면 사용 가능
    - `Collection` 인터페이스를 구현하지 않았더라도, 편한 순회를 위해 `Iterable` 구현을 고려하자.
  - 컬렉션인지, 배열인지와는 무관하게 사용
  - 반복 대상이 컬렉션인지 배열인지와 무관하게 속도 차이는 없음
  - 컬렉션을 중첩 참조할 경우, 향상된 `for`문이 갖는 강점이 더 극대화
- **향상된 `for` 문을 사용할 수 없는 상황**
  - **파괴적인 필터링(desctructive filtering)**
    - 컬렉션을 순회하면서 선택된 원소를 제거해야 할 때는 `Iterator.remove()` 호출하는 방식으로 구현해야 함
    - 자바8 이후에는 `Collection.removeIf()` 이용하여 컬렉션의 명시적 순회를 피할 수 있게 됨
  - **변형(transforming)**
    - 순회 도중 원소의 일부 값 혹은 전체를 교체해야 할 경우 인덱스가 필요
  - **병렬 반복(parallel iteration)**
    - 여러 컬렉션을 병렬 순회해야 할 경우
...
- 난수 생성의 예시
  ...
  - 제대로 된 난수 생성 메서드를 만들려면 의사난수 생성기, 정수론, 2의 보수 등 여러 주제에 대한 전문성이 필요하다.
  - 이미 만들어진 `Random.nextInt(int)` 메서드를 사용하면 위와 같은 지식 없이 필요한 기능을 바로 사용할 수 있다.
    - 자바7부터는 `ThreadLocalRandom`이 더 낫다.

- **표준 라이브러리 사용의 강점**
  - 위의 사례처럼, **표준 라이브러리를 사용하면 다른 프로그래머 / 전문가의 지식과 경험을 활용할 수 있다.**
  
  - **핵심 업무와 무관한 일을 해결하는 데 소요되는 시간을 절약할 수 있다.**
  
  - **성능이 지속적으로 개선된다.**
  
  - **기능이 지속적으로 추가된다.**
  
  - **내가 표준 라이브러리를 활용해서 작성한 코드는 다른 사람들이 이해하기도 쉽다.**
  
- 자바 프로그래머라면
  - `java.lang`, `java.util`, `java.io`와 그 하위 패키지에 익숙해져야 하며 이 외에도
  - 컬렉션 프레임워크, 스트림 라이브러리, `java.util.concurrent` 등도 능숙해지면 좋다.
- 표준 라이브러리가 필요한 기능을 제공해주지 못한다면 서드파티 라이브러리를 활용하자. (ex. `Guava`)
  - 서드파티 라이브러리에도 원하는 기능이 없다면 직접 구현한다.



## 아이템60. 정확한 답이 필요하다면 `float`와 `double`은 피하라

> **`float`과 `double`은 정밀한 금융 관련 계산에 부적합하다!**

- 빠르게 **근사치**를 계산하기 위해 설계되었기에 ***0.1 혹은 10의 거듭제곱 수를 표현할 수 없다.***

- 반올림을 이용하는 방식도 부적합하다.

- ***금융 계산에는 `BigDecimal`, ` int `, `long`을 사용하라!***
  ...
  - `BigDecimal`의 경우 생성자에서 문자열을 받고 있다: 정확성을 위함이다.
  - 위와 같은 `BigDecimal` 이용하는 방식은 **사용이 불편하며, 성능이 훨씬 느리다는 단점이 있다.**
...
- 개선: `int`나 `long`
  ...
  - 이 경우 **다룰 수 있는 값의 크기가 제한되며, 소수점을 직접 관리해야 한다는 점에 주의해야 한다.**
  - 위 코드에서는 달러 대신 센트로 수정하는 방식으로 해결하였다.



## 아이템61. 박싱된 기본 타입보다는 기본 타입을 사용하라

- 기본 타입과 박싱된 기본 타입은 오토방싱과 오토언박싱 덕분에 크게 구분하지 않고 사용할 수 있다.

- 그럼에도 둘 사이의 차이로 인해 주의해서 사용해야 한다.

- **기본 타입과 박싱된 기본 타입의 차이**

  - 박싱된 기본 타입은 식별성(identity)를 갖는다.
  - 박싱된 타입은 nullable
  - 기본 타입이 시간과 메모리 사용 면에서 보통 더 효율적이다.
...
- **박싱된 기본 타입의 주의사항**
  ...
  - **`==`을 사용하면 잘못된 비교 수행**
    ...
    - 0이 아니라 1을 출력한다: `i`와 `j`가 서로 다른 `Integer` 인스턴스이기에 기본 타입 값으로 변환하지 않고 객체 참조 동일성을 검사하기 때문이다.
    - 기본형 값으로 저장한 다음 연산을 수행해야 위 문제를 막을 수 있다.

  - **`null`인 박싱된 기본 타입과 기본 타입의 비교**
    ...
    - 위의 경우 NPE 발생
    - 기본 값과 박싱된 기본 타입을 비교할 경우 박싱된 기본 타입을 언박싱하는데,
      이 과정에서 NPE 발생했다.
  ...
  - **성능 문제**
    ...
    - `sum`이 `Long`으로 선언되어서 박싱과 언박싱이 반복적으로 일어나 심각한 성능 저하를 발생

- **박싱된 기본 타입을 써야 하는 경우**

  - 컬렉션의 원소, key, value로 사용: 컬렉션은 기본 타입을 담을 수 없기에
  - 매개변수화 타입이나 매개변수화 메서드의 타입 매개변수: 위의 이유이기도 함
...
- **문자열은 다른 값 타입을 대신하기에 부적합하다.**

- **문자열은 열거 타입을 대신하기에 부적합하다**: 상수 열거시에는 열거 타입이 훨씬 낫다.

- **문자열은 혼합 타입을 대신하기에 부적합하다**

  - 문자열 내 혼합 요소 구분자가 각 요소 내에 쓰였을 경우 혼란스러움
  - 개별 요소 접근 시에 문자열 파싱해야 해서 느리고 오류 가능성 상승
  - `equals`, `toString`, `compareTo` 등 적절히 사용 불가
  - 그냥 `private static` 멤버 클래스로 대체하는 게 나음

- **문자열은 권한을 표현하기에 부적합하다**
  ...
  - 스레드 구분용 문자열 키가 전역 네임스페이스에 공유됨
    - 각 클라이언트가 고유한 키를 제공해 줘야 하며, 공유된 키가 있을 겨웅 기능적 문제 발생
    - 악의적 클라이언트가 동일 키를 사용하여 다른 클라이언트의 값을 가져올 수 있음
  ...
  public class ThreadLocal {
      ...
      public static Key set(Key key, Object value);
  ...
  - 문자열이 아니라 권한 전용 클래스 만들어서 문제 해결

  ```java
  public final class ThreadLoacL<T> {
      public ThreadLocal();
  ...
  - 최종적 리팩토링
    - 구분자인 Key와 ThreadLocal 클래스를 분리할 필요가 없게 인스턴스 메서드로 변경
    - 매개변수화 타입으로 만들어서 타입 안정성 챙김



## 아이템63. 문자열 연결은 느리니 주의하라

- `+` 연산자로 문자열 `n`개를 연결하는 시간은 `n^2`에 비례
  - 문자열이 불변이기에 발생하는 문제
- `StringBuilder` 사용하는 것이 성능적으로 나음
...
- **인터페이스 참조가 더 낫다는 것은 매개변수뿐만 아니라 반환값, 변수, 필드 모두에 적용될 수 있음**
  - 다른 구현체로의 변환이 용이함
  - 단, 변환 이전의 구현체의 구현 방식에 의존적인 방식으로 코드를 짜 두었을 경우에는 주의가 필요
- **적합한 인터페이스가 없을 경우 클래스로 참조해야 한다.**
  - 값 클래스: `String`, `BigInteger`
    - 값 클래스가 여러 방식으로 구현되는 일은 드물기에, `final`로 설계되는 경우가 많음
  - 클래스 기반 프레임워크의 객체들: `java.io.OutputStream`
  - 인터페이스에 없는 특정한 메서드를 제공하는 클래스: `PriortyQueue`
    - `Queue`에는 없고 `PriortyQueue`에만 존재하는 메서드(`comparator`) 사용하기 위해서 구현 클래스를 참조해야 함
  - **인터페이스 참조가 아닌 경우에도, 가능한 한 덜 구체적인 클래스를 참조 타입으로 사용하는 편이 좋음**
    (객체 계층 그래프에서 가능한 한 위쪽을 참조하는 것이 좋다.)
...
- **리플랙션의 기능**
  ...
  - `java.lang.reflect`
  - 조회
    - 해당 클래스의 생성자, 메서드, 필드 인스턴스를 가져올 수 있음
    - 해당 클래스의 멤버 이름, 필드 타입, 메서드 시그니처를 가져올 수 있음
  - 조작
    - 생성자, 메서드 필드 조작 가능
    - `Method.invoke()`: 어떤 메서드든 호출 가능
  - 컴파일 당시에 존재하지 않던 클래스도 이용 가능

- **단점**

  - 컴파일타임 타입 검사의 이점을 누릴 수 없음
...
- **리플랙션을 사용해야 할 때?**
  ...
  - 리플랙션을 이용하는 프레임워크들 조차도 리플랙션 사용을 줄이고 있음: 아주 제한적인 형태로만 사용해야
  - **컴파일 타임에 이용할 수 없는 클래스를 사용해야 할 때**
    - 이런 경우라도 상위클래스나 인터페이스를 사용하는 편이 더 나음
    - 리플렉션은 해당 인스턴스 생성에만 사용하는 정도가 나음
  - 인터페이스로 참조해서 사용한다면, 해당 인스턴스의 사용에 있어서는 문제가 없음
    - 단, 리플렉션을 이용하여 해당 객체를 생성할 때는 리플렉션 특유의 여러 예외(`ReflectiveOperationException`으로 묶일 수 있음, since Java 7) 발생, 장황한 코드라는 단점을 겪을 것임
  - 런타임에 존재하지 않을 수도 있는 의존성을 관리하는 데 적합함
    - 해당 클래스, 메서드, 필드 등이 존재하는가를 검사하고 사용하는 식
    - 해당 기능들이 존재하지 않을 수 있음을 감안하고, 대체 수단을 마련해둔다.



## 아이템66. 네이티브 메서드는 신중히 사용하라

- **자바 네이티브 인터페이스(JNI; Java Nativa Interface)**
  - 자바 프로그램이 네이티브 메서드를 호출하는 기술
  - 네이티브 메서드
    - C / C++ 등 네이티브 프로그래밍 언어로 작성한 메서드
    - 주 용도
      - 플랫폼 특화 기능 사용: 레지스트리 등
        - 자바 버전이 상승하면서 **필요성 감소**
        - 자바9의 경우 `process` API 추가하여 OS 프로세스에 접근 가능하게 만듦
      - 네이티브 메서드로 작성된 기존 라이브러리 활용
      - 성능이 중요한 부분에 활용: **추천하지 않음!**
        - 자바 초기(3)버전이 아닌, 지금의 JVM은 성능에 있어서 크게 밀리지 않기 때문임
        - `BigInteger`의 사례
          - 1.1(`java.math` 첫 등장) 시점의 `BigInteger`는 C 라이브러리 의존
          - 자바3때 순수 자바로 구현, 튜닝을 통해 네이티브 구현보다 더 좋은 성능 발휘
          - 하지만 이후 자바로 만든 `BigInteger`의 경우 큰 변화가 없었음
            - 반면 네이티브 라이브러리의 경우 GNU 다중 정밀 연산 라이브러리(GMP)를 필두로 개선이 이루어짐
            - 고성능 정밀 연산을 요할 경우 네이티브 메서드 호출이 오히려 고려할만한 것이 되었음
- 네이티브 메서드의 단점: 안전성
  - 메모리 훼손 위험
  - 이식성이 낮음
  - 디버깅이 어려움
  - 경우에 따라 속도가 더 느려질 수 있음
  - GC가 추적 및 메모리 자동 회수를 수행하지 못함
  - 자바 코드 <-> 네이티브 코드 전환 시마다의 비용
  - 자바 코드 <-> 네이티브 코드 사이의 접착 코드(glue code)를 별도로 작성해줘야 함



## 아이템67. 최적화는 신중히 하라

- **최적화는 보통 안 좋은 결과로 이어진다.**
- **빠른 프로그램보다는 좋은 프로그램을 작성하라**
- **구현상의 설계를 미루라는 것이지, 설계 상에서 설계를 무시하라는 것은 아님!**
- **성능과 관련된 설계 원칙**
  - 성능을 제한하는 설계를 피하라
    - 컴포넌트 / 외부 시스템과의 소통 방식: API, 네트워크 프로토콜, 저장용 데이터 포맷 등
      변경이 어려운 요소들을 섣불리 설계한 경우 성능을 심각하게 제한할 수 있음
  - API 설계 시 성능 영향을 고려하라
    - `public` 타입을 가변으로 만듦 -> 불필요한 방어적 복사에 비용 소모
    - 컴포지션이 아닌 상속으로 설계한 클래스 -> 상위 클래스의 성능 제약을 상속
    - 인터페이스가 아닌 구현을 사용 -> 특정 구현체의 성능 제약 종속
    - **성능을 위해 API를 왜곡하지 말자**
      - 해당 성능 문제는 다음 버전에서 해결될 수 있지만,
        왜곡된 API로 인한 문제는 계속될 것이다.
  - **좋은 구조를 갖춘 프로그램을 갖춘 이후에 최적화를 고려해보자**
  - **최적화 시도 이후에 성능 측정을 통해 검증하라**
    - 이 문제는 C / C++같은 언어보다도 자바에서 더 중요하다: 성능 모델이 덜 정교하기 때문
    - 프로파일링 도구를 활용하자, jmh(자바 코드 성능 확인용 마이크로 벤치마킹 프레임워크)도 좋음



## 아이템68. 일반적으로 통용되는 명명 규칙을 따르라

- 자바 플랫폼 명명 규칙의 대부분은  JLS 6.1에 명시되어 있음
- 철자와 문법 규칙으로 구분
...
- 패키지, 클래스, 인터페이스, 메서드, 필드, 타입 변수의 이름을 규정
- 특별한 이유가 없는 한 반드시 따라야 함
- **패키지와 모듈 이름**
  - 소문자 및 알파벳으로 구성된 각 요소를 점(`.`)으로 구분하여 계층적으로 명명
  - 조직 외부에서 사용될 패키지의 경우 도메인 명을 역순으로 사용: `com.google`
    - 예외) 표준 라이브러리 및 몇몇 패키지들은 `java`, `javax`로 시작함
  - 패키지 이름의 각 요소는 일반적으로 **8자 이하의 짧은 하나의 단어**로 구성: `utilities`보다는 `util`, `awt` 등 약어 사용도 좋음
- **클래스 및 인터페이스 이름**
  - 대문자로 시작
  - 매우 널리 통용되는 경우가 아니면 약자를 사용하지 않음
    - 약자의 첫 글자만 대문자로 할지, 전체를 대문자로 할지는 논란의 여지 있음
    - 첫 글자 쪽이 더 우세: 해당 약자의 시작과 끝을 명확히 알기 좋음: `HttpUrl`
- **메서드 및 필드 이름**
  - 첫 글자가 소문자라는 점만 제외하면 클래스 및 인터페이스와 동일
  - 예외: 상수 필드: `NEGATIVE_INFINITY`
  - 지역변수의 경우 맥락상 유추가 용이하기에 약어 사용해도 좋음
  - 입력 매개변수의 경우 지역변수와 유사하지만, 메서드 문서화 대상이기에 좀 더 신경써야함
  - 타입 매개변수의 경우 보통 한 문자로 표현
    - 임의의 타입  `T`
    - 컬렉션 타입의 원소 `E`
    - 맵의 키와 값 `K`, `V`
    - 메서드의 반환 타입  `R`
    - 타입 시퀀스 `T`, `U`, `V` 혹은 `T1`, `T2`, `T3`



### 문법 규칙

- 철자 규칙에 비해 유연하고 논란 있음
- 클래스 및 인터페이스
  - 객체를 생성할 수 있는 클래스 및 열거형의 명칭은 단수 명사나 명사구
  - 객체를 생성할 수 없는 클래스의 명칭은 복수형 명사
  - 인터페이스의 명칭은 클래스와 유사한 방식으로 명명하거나  `-able`, `-ible` 형태 형용사로 명명
- 메서드
  - 동작을 수행하는 메서드: 동사나 목적어를 포함한 동사구
  - `boolean` 반환하는 메서드의 경우 `is` 혹은 `has`로 시작, 명사 / 명사구 혹은 형용사로 기능하는 단어나 구로 끝냄
  - 해당 인스턴스의 속성을 반환하는 메서드의 경우 명사 /명사구 혹은 `get`으로 시작하는 동사구
  - 타입을 변환하는 메서드의 경우  `toType` 형태로 명명
  - 다른 뷰로 보여주는 메서드의 경우 `asType`으로 명명
  - 기본 자료형으로 변환하는 경우 `typeValue` 형태로 명명(`intValue`)
  - 정적 팩터리 메서드의 경우 아이템1 참고
- 필드
  - 덜 명확하고 덜 중요: 노출되지 않는 것이 좋은 설계이기 때문
  - `boolean` 타입의 경우 관련 접근자 메서드에서 앞 단어를 뺀 형태
  - 이 외의 경우 명사나 명사구
...
- 예외를 흐름 제어에 사용한 케이스
    ...
    - 코드의 가독성이 안 좋음
    - JVM은 배열에 접근할 때마다 인덱스 경계를 넘었는지 검사하는데, 해당 검사의 수를 줄여보려고 예외를 이용하여 흐름 제어한 코드(실제로는 JVM이 알아서 최적화)
    - 코드를 `try-catch` 블록 안에 넣을 경우 JVM이 수행할 수 있는 최적화가 제한됨
    - 예외 자체가 예외 상황을 위해 만들어졌기 때문에 최적화가 덜 되어 있음

- **예외는 오직 예외 상황에서만 써야 함. 결코 일상적인 제어 흐름용으로 사용되어선 안 됨**

    - 복잡한 테크닉을 이용한 최적화보다는 표준적이고 가독성 좋은 관용구를 사용하는 게 나음
    - 최적화는 자바 플랫폼에 의해 꾸준히 개선되지만, 복잡한 테크닉을 이용해서 발생한 유지 보수 문제는 지속될 것이기 때문

- **잘 설계된 API라면 클라이언트가 정상적인 제어 흐름에서 예외를 사용할 일이 없어야 함**

    - 상태 의존적 메서드를 제공할 경우 상태 검사 메서드를 함께 제공해야 함
        ...
        - 나쁜 케이스: 상태 검사 메서드(`hasNext()`가 없다면)
            ...
            } catch (NoSuchElementExcpeion e) {
              
            }
            ```

    - 상태 검사 메서드의 대안: 빈 `Optional`, `null` 등 특정 값 사용하기

        - 동기화 없이 여러 스레드가 접근 가능하거나 외부 요인으로 상태가 변할 수 있는 경우 특정 값을 사용
            - 상태 검사 메서드 - 상태 의존 메서드 호출 사이에 상태 값이 변경될 수 있기 때문
        - 성능이 민감한 상황에서 상태 검사 메서드가 작업을 중복 수행하는 경우 특정 값을 사용
        - 일반적인 경우 상태 검사 메서드 방식을 사용
            - 가독성이 좋으며 문제 발생 시 확인이 빠르기 때문임
...
1. **호출 측에서 복구할 것으로 여겨질 경우 검사 예외 사용**
	- 호출자는 검사 예외를
		1. catch로 잡아 처리하거나
		2. 더 바깥으로 전파한다.
	- 검사 예외는 그 상황에서 **호출자에게 복구할 것을 요구**하는 것이다.
...
- 예외 정의하기
    - 비검사 throwable은 모두  `RuntimeException`을 상속한 클래스로 만들 것
    - 예외 역시 객체이기에 검사 예외의 경우 호출자가 해당 예외 상황을 해결할 수 있는 정보를 주는 메서드를 함께 제공해주면 좋음
...
- 하지만, 검사 예외를 남용할 경우 API 사용을 불편하게 한다.
	- 특히 검사 예외를 던지는 메서드는 스트림 내에서 직접 사용할 수 없다.
...
- 대안
	...
	- 검사 예외를 던지는 메서드를 비검사 예외 + 메서드2개로 쪼개는 방식: `Boolean` 값 던지는 메서드, 외부에서 처리하는 메서드
...
- 주요 목록
	...
	- `UnsupportedOperationException`: 호출된 메서드를 지원하지 않음
		- 일반적인 경우 정의한 메서드를 모두 지원하기 때문에 사용 적음
...
- 모호한 경우
	- 인수 값이 무엇이었든 어차피 실패했을 경우에는 `IllegalStateException`, 그렇지 않으면 `IllegalArgumentException`을 사용하라.



## 아이템73. 추상화 수준에 맞는 예외를 던져라
- 저수준 예외를 처리하지 않고 바깥으로 전파시키면
...
- 검사 예외는 항상 따로 선언하고, 각 예외 상황을 `@throws` 태그로 명확히 문서화해야 함
    - `throws Exception` 등으로 퉁쳐서 선언해서는 안 된다는 의미임
    - 단 `main()` 메서드의 경우 당연히 예외임
- 비검사 예외라 할지라도 문서화해두면 좋음
    - 잘 정리된 비검사 예외는 해당 메서드의 전제조건 명세와 같음
    - 인터페이스 메서드의 경우 이 중요성이 더 커짐, 해당 인터페이스를 구현한 경우 모두에 일관적으로 동작하게 만들어주기 때문
    - 비검사 예외의 경우 메서드 시그니처의 `throws` 목록에 넣어선 안 됨
        - JavaDoc 문서에서도 `throws`에 명시된 예외를 `@throws` 명시한 예외 중에서 구별해줌
- 해당 클래스 내 다수의 메서드가 동일한 이유로 동일한 예외를 던질 경우 클래스 설명에 예외 설명을 추가해줄 수 있음: `NullPointerException`이 대표적인 예시임
...
- Stack trace
    - 자바 프로그램이 예외를 잡지 못해 실패할 경우 자동으로 stack trace 출력
    - 예외 객체의 `toString()` 호출하여 얻어지는 `String`
    - 일반적으로는 `예외 클래스 이름 + 상세 메시지` 형태임
    - 사후 분석을 위해 상세 메시지에 원인에 대한 정보를 가능한 한 많이 담아야 함
- **예외 상세 메시지 작성 팁**
    - 원인 추적을 위해 관련된 모든 매개변수와 필드 값을 상세 메시지에 담아야 함
    - 관련된 데이터를 모두 담되, 해당 소스코드 및 문서에서 얻을 수 있는 정보를 굳이 기록할 필요는 없음
    - 상세 메시지 != 최종 사용자가 볼 오류 메시지
        - 상세 메시지를 볼 사람이 누구인가를 생각해보자: 가독성보다는 내용이 더 중요함
    - 예외 생성자에서 연관된 정보를 모두 받아서 상세 메시지를 생성하는 것도 좋음
        - 이런 정보에 대해 접근할 수 있는 방법을 해당 예외 클래스에 만들어 두는 것도 좋음
...
- 실패 원자성(failure-atomicity)
    ...
    - 메서드 명세에 기술된 예외의 경우 객체 상태의 실패 원자성을 보장해야 한다
    - 보장하지 못할 경우 실패 시의 객체 상태를 API 문서에 명시해야 한다

- 실패 원자성을 얻는 방법
    - 객체 자체를 불변 객체로 설계하기
    - 작업 수행 전 매개변수 유효성 검사하기
        - 잘못된 매개변수로 인한 작업 수행으로 인해 객체가 비정상적인 상태에 빠지는 것을 방지
        - 실패 가능성이 있는 모든 코드를 상태 변경이 발생하는 코드보다 앞에서 수행
    - 객체의 임시 복사본에서 작업 수행 후 작업 완료 이후에 원래 객체와 교체하기
        - 임시 자료구저에서 작업하는 것이 더 빠를 경우 적용: 이 경우 성능 향상까지 꾀할 수 있음
    - 작업 도중 실패 발생을 `catch`하고 복구 코드를 이용하여 작업 전상태로 되돌리기
        - 내구성을 보장해야 하는 자료구조에서 고려해볼 수 있음
- 실패 원자성을 달성할 수 없는 경우
    - 동시성 문제: `ConcurrentModificationException`을 잡아냈다고 해서 객체의 상태가 훼손되지 않았음을 보장할 수 없음
    - `Error`: 복구 자체가 불가
- 실패 원자성을 구현할 수 있음에도 포기해야 할 때: 지나치게 비용이 클 경우
...
- 아무 작업도 수행하지 않는 `catch` 블록이 대표적이다.
...
- 예외를 무시해야 할 경우
    ...
    - `catch` 블록 안에 무시하는 이유를 명시해야 함
    - 예외 변수의 이름을 `ignored`로 바꿔 두어야 함
    - 예시) `FileInputStream`을 닫을 때 예외 처리
        - 입력 스트림을 닫는 도중이기에, 복구할 것이 없는 상황임
        - 닫는 상황은 이미 필요 정보를 다 읽었음을 의미하기에 남은 작업도 없음 == 중단할 작업 없음
        - 단, 해당 상황이 자주 발생한다면 조사가 필요하기에 이런 경우에도 로그는 남기는 편이 나음
...
1. **배타적 실행(mutual exclusion)**
    - 한 스레드가 변경중인 객체를 다른 스레드가 보지 못하게 막고(객체 단위의 lock),
        필요에 따라 객체의 상태를 바꿈
        (a state transition, transforming the object from one consistent state to another.)
    - = 일관성이 깨진(inconsistent) 상태의 객체를 드러내지 않음

2. **스레드 간 변화 공유(reliable communication)**
    - 한 스레드가 만든 변화의 결과를 다른 스레드가 확인할 수 있게 만듦
        (but it ensures that each thread entering a synchronized method or block sees the effects of all previous modifications that were guarded by the same lock.)



### 원자적 데이터와 동기화 - 스레드 간 통신문제

#### `long`과 `double` 값을 쓰는 작업은 원자적이지 않다

> ## 17.7. Non-Atomic Treatment of `double` and `long`
>
> For the purposes of the Java programming language memory model, a single write to a non-volatile `long` or `double` value is treated as two separate writes: one to each 32-bit half. This can result in a situation where a thread sees the first 32 bits of a 64-bit value from one write, and the second 32 bits from another write.
>
> 자바 언어 메모리 모델의 목적으로 인해, `volatile`이 아닌 `long`이나 `double` 값에 대한 하나의 쓰기(single write) 연산은 두 개의 분리된 쓰기 연산(two separate writes)으로 취급된다. 이로 인해 64bit 값을 다를 때, 한 스레드가 어떤 쓰기 연산에서는 64비트 중 앞의 32비트를 다루게 되고, 다른 쓰기 연산에서는 64비트 중 뒤의 32비트를 다루게 되는 일이 발생할 수 있다.
>
> Writes and reads of volatile `long` and `double` values are always atomic.
>
> `volatile`인 `long`과 `double` 값에 대한 읽기 / 쓰기의 경우 항상 원자적(atomic)이다.
>
> Writes to and reads of references are always atomic, regardless of whether they are implemented as 32-bit or 64-bit values.
>
> 참조 값(references)에 대한 읽기와 쓰기의 경우 해당 값이 32비트로 구현되었건, 64비트로 구현되었건 항상 원자적이다.
>
> Some implementations may find it convenient to divide a single write action on a 64-bit `long` or `double` value into two write actions on adjacent 32-bit values. For efficiency's sake, this behavior is implementation-specific; an implementation of the Java Virtual Machine is free to perform writes to `long` and `double` values atomically or in two parts.
>
> 몇몇 (JVM) 구현체들은 64비트 `long`이나 `double` 값에 대한 한 번의 쓰기 작업을 인접한(adjacent) 32비트 값에 대한 두 번의 쓰기 작업으로 나누어 다루는 쪽을 더 편리하게 여길 수 있다. 효율성을 위해 위와 같이 동작하는지 여부는 구현체에 따라 다를 수 있다. JVM 구현체는 `long`이나 `double` 값에 대한 쓰기 작업을 원자적으로 수행할 수도, 둘로 나누어 수행할 수도 있다.
>
> Implementations of the Java Virtual Machine are encouraged to avoid splitting 64-bit values where possible. Programmers are encouraged to declare shared 64-bit values as `volatile` or synchronize their programs correctly to avoid possible complications.
>
> JVM 구현체들은 가능한 한 64비트 값을 나누어(splitting) 다루지 않는 편이 권장된다. 위와 같이 복잡한 문제를 피하기 위해서 프로그래머들의 경우 공유되는 64비트 값들을 `volatile`로 다루거나 동기화하는 것이 권장된다.



#### Read / Write가 원자적인 값들의 동기화

- `long`, `double`가 아닌 변수를 읽고 쓰는 작업은 모두 원자적이다. [JLS 17.4]
- ***그럼에도 동기화가 필요하다는 것에 주의하라!***
    - 변수를 읽고 쓰는 동작이 원자적임 == 배타적 실행을 보장함,
        변수를 읽고 쓰는 동작이 원자적임 **!= 스레드 간 변화 공유를 보장함**
    - 스레드가 필드를 읽을 때 **변경이 완료된, consistency가 보장된 값**을 얻음은 보장되지만,
        **다른 스레드가 그 필드를 읽을 때 해당 변경이 완료된 값이 보임은 보장되지 않음**
    - 자바 메모리 모델 규정에 의함



### 배타적 실행이 보장되더라도 스레드 간 통신에 실패한 사례
...
- 다른 스레드를 멈추는 방법
    ...
    - `Thread.stop()`은 사용해선 안 된다!
    - `boolean` 필드를 이용하여 멈출 수 있다, 하지만 위와 같은 코드는 정상 작동하지 않는다!
        -> `boolean` 필드를 동기화할 필요 있음! (쓰기 말고 읽기 차원에서도 동기화 필요)

- 정상 작동하지 않는 이유

    - 다른 스레드(메인 스레드)에서 `boolean` 값을 수정한 바가 `backgroundThread`에 언제 전달되지가 보증되지 않기 때문이다.

    - JVM의 최적화로 인해 다음과 같은 형태로 변경된다:
        ...
        - OpenJDK 서버 VM이 적용하는 `hoisting` 기법
...
- `volatile`
    ...
    - 주의사항
        ...
        - 위의 코드는 동기화가 정상적으로 이루어지지 않는다!
        - 원인은 증가 연산자 `++`
            - `++`연산자는 값에 두 번 접근한다:
                - 해당 값을 읽기 위해 1번
                - 증가된 값을 저장하기 위해 1번
        - 만일 다른 스레드가 읽기 - 새 값 저장의 두 작업이 일어나는 사이에 접근할 경우 문제 발생한다: 안전 실패(safety failure)
        - `synchronized` 메서드로 붙여 해결 가능하다.
            - 이 경우 `volatile` 키워드는 뺀다.
...
- `java.util.concurrent.atomic` 패키지
    ...
    - `AtomicLong` 등 제공
    - 락 없이도(lock-free) 스레드 안정성을 제공한다.
    - 배타적 실행 + 스레드 간 통신 모두를 지원한다.
    - `synchronized` 보다 성능도 우수하다.



### 정리

- **최선의 방법은 그냥 가변 객체 자체를 공유하지 않는 것임**
    - 불변 객체만 공유하거나 아무 것도 공유하지 말자
    - = 가변 데이터는 단일 스레드에서만 쓰자
    - 위와 같은 정책 설정 시 문서화하여 이후에도 준수되게 만들어야 한다
- 라이브러리 / 프레임워크가 스레드를 이용하여 동기화 문제를 일으킬 수 있는지 이해하는 것도 중요
- 객체의 일부분만 공유해야 할 경우
    - 공유 부분만 동기화해도 된다 -> 사실상 불변(effectively immutable)인 객체
    - 안전 발행(safe publication): 사실상 불변인 객체를 다른 스레드에 넘기는 것
    - 일부분 공유하는 여러 방법
        - `static` 필드, `volatile` 필드, `final` 필드, 락을 통해 접근하는 필드 사용
        - `concurrent` 지원하는 컬렉션 이용하기



 ## 아이템79. 과도한 동기화는 피해라

- 과도한 동기화가 야기할 수 있는 문제점
    - 성능 저하
    - 교착 상태 발생
    - 예측할 수 없는 동작
...
### 동기화 메서드 / 블록 내에서는 제어를 양도해선 안 된다

- 동기화 메서드 / 블록 내에서 클라이언트가 제어를 이어 받을 경우 응답 불가와 안전 실패 가능성 있음

    - `final` 메서드가 아닌 메서드를 호출해선 안 됨
    - 전달 받은 함수 객체를 실행해선 안 됨
    - 외부 메서드(alien method)의 동작을 예측할 수 없기 때문임
...
- `ObserverableSet<E>`이 사용하는 콜백 함수형 인터페이스 `SetObserver<E>`
...
- 여담) 위의 경우 자신을 참조하기 위해 람다가 아닌 익명 클래스를 사용하였음
- 위의 경우에는 정상 작동하지 않는다.
    - 23까지 출력한 이후에 `ConcurrentModificationException` 발생
    - `ObservableSet<E>`이 `observers`를 순회하는 도중에 위의 `observer`의 `added()`가 호출된다.
    - `observers` 순회 도중에 `observers`에 영향을 주는 `removeObserver()`가 호출되어 버린다: **순회 중인 리스트의 원소가 삭제 시도된다.**
- `synchronized`가 외부 스레드와의 동기화는 수행해주지만,
    내부에서의 콜백으로 인한 동시성 문제는 해결해주지 못하는 상황이다.
...
- 백그라운드 스레드는 `s.removeObserver()` 실행을 위해 `observers` 잠그려고 시도하지만 락을 얻을 수 없음, 
    메인 스레드가 이미 락을 가지고 있기 때문.

- 메인 스레드는 백그라운드 스레드가 `observer` 삭제하기를 기다리기에 교착 상태 발생



#### 만약 불변식이 깨진다면?

- 위의 예외 / 교착상태 발생 케이스의 경우 `observers`가 호출 시점에 일관된 상태였음
- 위와 달리 불변식이 임시로 깨진 케이스의 경우, 
    자바의 락이 재진입(reentrant)을 허용하기에 교착상태에 빠지지는 않지만 안전 실패(데이터 훼손)이 발생
    - 자바의 락 시스템은 교착 상태를 안전 실패로 바꿀 수 있음에 유의해야 한다.
...
- 열린 호출(open call)
    ...
    - 외부 메서드의 호출을 동기화 블록 밖으로 옮기는 방식
    - 동기화 영역 바깥에서 호출되는 외부 메서드를 지칭함
    - 위의 문제를 해결 가능: 위의 경우에서는 `observers`를 복사해 사용
    - **동기화 영역에서는 일을 최소화하는 것이 중요**

- `CopyOnWriteArrayList`

    - 위의 용도를 위해 만들어진, `ArrayList`의 구현체
    - 내부 변경 작업은 복제를 통해 실행
    - 순회 시 락이 불필요하기에 속도 빠름, 수정이 드물고 순회만 자주 발생한다면 성능 보장
...
- 성능: 과도한 동기화는 멀티코어 활용을 막고 가상 머신의 코드 최적화를 제한한다는 점에서 주의 필요
- 가변 클래스 작성 시 지침
    - 동기화 하지 않기, 해당 클래스를 외부에서 사용할 때 동기화하게 만들기
        - `Vector`와 `HashTable` 제외한 `java.util`에서 사용하는 방식
    - 동기화를 내부에서 수행 -> 스레드 안전한 객체 만들기 (단, 동기화하지 않는 방식보다 성능 우월할 때만 사용)
        - `java.util.concurrent`에서 사용하는 방식
    - 지침 위배 케이스
        - `StringBuffer`: 단일 스레드 내에서 쓰임에도 내부적으로 동기화 수행
            -> `StringBuilder` 등장
        - `java.util.Random` -> `java.util.concurrent.ThreadLocalRandom`도 마찬가지
        - 동기화해야할 지 애매한 상황이면 동기화하지 말고 문서에 스레드 안전하지 않음을 명시하는 편이 나음
...
- 실행자 프레임워크(Executor Flamework)
    ...
    - 제공 기능
        ...
        - 특정 task 완료를 기다림
        - 아무 task 하나가 완료되기를 기다림 (`invokeAny()`) / 모든 task 완료를 기다림 ( `invokeAll()` )
        - 실행자 서비스 종료를 기다림 ( `awaitTermination()` )
        - 완료된 tasks 결과 차례로 받기 ( `ExecutorCompletionService` )
        - 특정 시간 혹은 주기적으로 task 실행 ( `ScheduledThreadPoolExecutor` )

- 주의사항

    - 작은 프로그램이나 가벼운 서버의 경우 `Executors.newCachedThreadPool` 사용
        - 큐에 task 쌓지 않고 바로 스레드에 위임됨
        - 사용할 스레드가 없을 경우 새로 생성
        - 이로 인해 무거운 서버의 경우 CPU 100% 사용 부딪히며 무한히 많은 스레드 생성함
    - 무거운 서버의 경우 `Executors.newFixedThreadPool` 이용하거나 (스레드 개수 고정)
        `ThreadPoolExecutor` 이용 (통제 가능)
    - 작업 큐 / 스레드 직접 다루는 일은 피하는 것이 좋음
        - 이 경우 `Thread`가 작업 단위 / 수행 메커니즘 둘 다 수행함
        - Executor Flamework 사용 시 둘은 분리됨
            - task: 작업 단위
                - task의 두 종류 `Runnable`, `Callable` (`Callable`의 경우 값 반환 및 예외 던지기 가능)
            - `ExecutorService`: 수행 매커니즘
    - 포크-조인(fork-join) 태스크
        - Since Java7
        - `ForkJoinTask`는 하위 task로 분할 가능
        - `ForkJoinPool` 스레드가 분할된 task들을 처리, 다른 스레드의 task 접근하여 대신 수행도 가능
        - 병렬 스트림이 해당 방식 이용
...
- `wait`와 `notify`는 사용이 까다롭기에 고수준 동시성 유틸리티를 사용하자.
- `java.util.concurrent`의 고수준 유틸리티 분류
    - 실행자 프레임워크
    - 동시성 컬렉션
    - 동기화 장치(synchronizer)
...
- 내부적으로 동기화 수행 -> 동시성 무력화 불가
- 외부에서 락을 사용하면 성능만 떨어짐

- 상태 의존적 수정 메서드

    - 여러 동작을 원자적 단위로 묶음

    - 내부적 동기화로 인해 여러 동작을 원자적 단위로 묶어 호출할 수 없다는 점 때문에 추가됨

    - `Map.putIfAbsent(key, value)`

        - `String.intern()`을 흉내낸 코드의 예시

            - `String.intern()`: Constant Pool에 해당 `String` 리터럴 있을 경우에는 Constant Pool에서 가져오고, 아닐 경우 Constant Pool에 해당 리터럴 추가
            ...
            public static String intern(String s) {
                String priviousValue = map.putIfAbsent(s, s);
                return previousValue == null ? s : previousValue;
            }
            ```
        
    - 단 위 구현의 경우 `get()` 연산에 최적화된 `ConcurrentHashMap`의 성능을 제대로 활용하고 있지 않기에 다음과 같은 최적화가 가능
...
- 스레드가 다른 스레드를 기다릴 수 있게 하여 작업 조율
- 종류
    - `CountDownLatch` : 사용 많음
    - `Semaphore` : 사용 많음
    - `CyclicBarrier`
    - `Exchanger`
    - `Phaser` : 기능적으로 강력
...
- 하나 이상의 스레드가 다른 스레드의 작업이 끝날 때가지 기다리게 하는 일회성 장벽
- `int` 생성자로 받는 값은 `countDown()` 메서드를 몇 번 호출해야 대기 중인 스레드를 깨울 수 있는가를 설정하는 값

- 예시 코드

    ```java
    public static long time(Executor executor, int concurrency, Runnable action) throws InterrupedException {
        ...
        for (int i = 0; i < concurrency; i++) {
            executor.execute( () ->{
                ...
                try {
                    ...
                    //done 타이머에게 작업을 마쳤음을 알린다.
                    done.countDown();
                }
            });
        }
        
        ready.await();			//모든 작업자가 준비될때까지 대기
        long startNanos = System.nanoTime();
        start.countDown();		//작업자들을 깨운다. (작업 시작)
        done.await();			//모든 작업자의 작업이 완료될 때까지 기다림
        
        return System.nanoTime() - startNanos;
    }
    ```

    - 위 예시의 경우 3번의 작업이 동시에 수행됨
        따라서 `executor`가 3개까지의 스레드를 생성할 수 있어야 함
        -> 아닐 경우, 영원히 끝나지 않는 코드임: 스레드 기아 교착 상태(thread starvation deadlock)
    - `InterruptedException` `catch` 시에는 `Thread.currentThread().interrupt()`로 인터럽트를 되살리고, `run()`에서 빠져 나옴 ( `executor`가 인터럽트를 적절하게 처리할 수 있게 만들기 위함 )
    - `System.nanoTime()`이 `System.currentTimeMillis()`보다 좋음
        - 더 높은 정확성을 가지며 시스템 실시간 시계의 시간 보정 영향 받지 않음
        - 정밀한 시간 측정이 필요한 경우에는 jmh 등의 특수 프레임워크를 쓰는 편이 더 나음



### `wait()` / `notify()`

- `wait()` / `notify()`보다 동시성 유틸리티가 우월하지만, 사용해야 하는 상황(레거시 코드 등)의 경우 주의사항을 알아야 함

- **`wait()`**: 스레드가 특정 조건이 충족될 때까지 대기하게 만듦

    

#### 주의사항

- **락 객체의 `wait()`메서드 호출은 반드시 해당 객체를 잠근 동기화 영역 안에서 호출해야 함!**
...
- **`wait()`메서드의 경우 위와 같은 대기 반복문 관용구 안에서만 사용하라. 반복문 밖에서는 절대로 호출해선 안 된다.**
- `wait()` 전에 조건 충족되었을 경우 `wait()`을 건너 뛰게 구현하라: 응답 불가 상태를 예방하기 위함
    - 조건 이미 충족 -> `notify()` / `notifyAll()` -> `wait()` 구조의 경우 스레드를 다시 깨울 수 있다고 보장 못 함
- `wait()` 이후에 조건 충족되지 않을 경우 `wait()`하게 만들라: 안전 실패를 막기 위함,
    예시:
    - `notify()`로 인해 스레드가 깨어나는 사이에 다른 스레드가 락을 얻어 상태 보호
    - 조건 불만족 상태임에도 다른 스레드가 실수 혹은 악의적으로 `notify()` 호출
        - 외부 노출된 객체의 동기화된 메서드 내에서 `wait()` 호출될 경우 모두 이 위험 있음
    - 깨우는 스레드가 지나치게 관대하여, 일부 스레드만 조건 충족되었음에도 `notifyAll()` 호출하는 경우
    - 허위 각성(spurious wakeup) 현상: `notify()` 호출 없이도 대기 중인 스레드가 깨어나는 현상



#### `notify()` vs `notifyAll()`

- `notify()`는 단일 스레드를 깨움, `notifyAll()`은 모든 스레드를 깨움
- ***`notifyAll()`을 사용하는 것이 합리적 / 안전함***
    - 모든 스레드가 깨어남을 보장하기 때문
    - `wait()`이후에 조건 충족 여부를 검사하게 구현할 경우 깨어나지 않을 스레드가 깨어나는 문제방지 가능 (위의 지나치게 관대한 스레드 문제의 해결)
        - 이러한 구현 방식은 악의적인 `wait()` 호출에 대한 방지도 되기 때문에 그냥 `notifyAll()` 사용하자



## 아이템82. 스레드 안정성 수준을 문서화하라

- API 문서에 스레드 안정성을 제대로 명시하지 않을 경우
    동기화가 부족하거나, 동기화가 지나치게 된 문제가 발생하게 됨
- **메서드 시그니처에 `synchronized`가 붙었다고 스레드 안정성 정보가 전달되지는 않음!**
    - JavaDoc API 문서에는 `synchronized` 붙어 있지 않음 : 해당 부분은 구현 이슈일뿐 API 명세가 아니기 때문
    - `synchronized` 외에도 여러 단계의 스레드 안정성이 존재함: 명확한 스레드 안정성 표기함이 낫다.
...
- 무조건적 스레드 안전(unconditionally thread-safe) `@ThreadSafe`
    - 수정 가능한 인스턴스지만, 내부적 동기화로 인해 외부 동기화 없이도 스레드 안전함
    - `AtomicLong`, `ConcurrentHashMap`
- 조건부 스레드 안전(conditionally thread-safe) `@ThreadSafe`
    - 일부 메서드의 경우 동시에 사용하려면 동기화가 필요
    - `Collections.synchronized` 메서드가 반환한 컬렉션
- 스레드 안전하지 않음(not thread-safe) `@NotThreadSafe`
    - 수정될 수 있는 인스턴스, 동시에 사용하려면 메서드 호출을 외부 동기화 매커니즘으로 감싸야 함
    - `ArrayList`, `HashMap`
- 스레드 적대적(thread-hostile) `@NotThreadSafe`
    - 모든 메서드 호출을 외부 동기화로 감싸더라도 멀티 스레드 환경에서 안전하지 않음
    - 일반적으로 정적 데이터를 동기화 없이 수정하는 경우 발생
    - 일반적으로는 수정되어 재배포되거나, deprecated API로 지정됨
...
- 조건부 스레드 안전한 클래스의 문서화
    - 어떤 순서 호출 시에 외부 동기화를 지원하는가
    - 그 순서로 호출하려면 어떤 락을 획득해야 하는가
        - 경우에 따라 그 인스턴스가 아닌 락을 얻어야 할 경우도 있음: `Collections.synchronizedMap` 경우
- 주석 기재 위치
    - 일반적으로는 클래스의 문서화 주석
    - 특수한 메서드의 경우 해당 메서드의 주석
    - `enum`의 경우 기재하지 않아도 됨(불변)
    - 정적 팩터리 메서드의 경우에도 반환 타입의 안정성 기재
- 외부 사용 가능한 락 제공하기
    - 제공 시 클라이언트에서 원자적 작업 호출 구현이 가능
    - 단점: 내부적 동시성 제어 메커니즘과 병행 불가(일반적으로 성능 희생)
    - DoS 공격 위험성: 클라이언트가 공개 락을 오래 쥐고 놓지 않는 경우



#### 비공개 락

- DoS 공격 예방: `synchronized` 메서드 대신 비공개 락 객체 사용
...
- 캡술화 되어서 공개되지 않고, 클라이언트가 동기화에 관여할 수 없기에 공격되지 않음

- 위의 경우 락 필드를 `final`로 선언하여 락 객체가 교체되는 상황 자체를 방지함
    -> **락 객체는 항상 `final`로 선언하라!**

- 위 방식은 무조건적 스레드 안전 클래스에서만 사용 가능:
    조건부 스레드 안전 클래스에서는 필요한 락 정보를 전달해야 하기 때문임

- 비공개 락은 상속용으로 설계한 클래스에 특히 적합

    - 해당 클래스에서 자기 자신의 인스턴스를 락으로 사용할 경우 하위 클래스가 기반 클래스의 동작을 방해하기 쉽기 때문임 
        -> 하위 클래스와 기반 클래스가 서로를 훼방하는 사태 발생 (`Thread` 클래스에서 종종 발생함)
...
- 필드 초기화 시점을 값이 필요한 시점까지 미루는 기법
- 용도
    - 일반적으로는 최적화
    - 클래스 / 인스턴스 초기화 시에 발생하는 순환 문제 해결
- 사용시 주의사항
    - 필요하지 않을 경우 사용하지 않는 것이 기본
        초기화 비용은 줄이지만, 해당 필드 접근 비용은 커지기 때문에 성능 향상을 확신할 수 없음
    - 해당 필드의 사용률이 낮지만, 필드 초기화 비용이 높을 경우 : 성능 측정을 통해 이 케이스인지 확인 필요
    - 멀티스레드 환경의 경우 반드시 동기화 필요
...
- 인스턴스 필드의 초기화
    ...
    - 일반적인 초기화
        ...
        ```java
        private final FieldType field = coputeFieldValue();
        ```
...
- **성능 향상을 위한 필드 초기화 기법**
    ...
    - **지연 초기화 홀더 클래스(lazy initialization holder class) 관용구**
        ...
        - 클래스가 처음 사용될 때 초기화된다는 특성을 이용하였음
        - `getField()`가 필드 접근하면서 동기화하지 않기에 성능상의 불이익이 없다는 강점
            (일반적인 VM의 경우 클래스 초기화 시에만 필드를 동기화하기에 가능)
    ...
    - **이중검사(double-check) 관용구**
        ...
        private FieldType getField() {
            ...
            if (result != null) {			//1차 검사 : 락 사용하지 않는 검사
                return result;
            }
            
            synchronized (this) {
                if (field == null) {		//2차 검사 : 락 사용
                    field = computeFieldValue();
                }
                return field;
            }
        ...
        - 동기화 없이 1회 검사 / 동기화하여 2회 검사를 통해
            만약 필드가 이미 초기화되어있을 경우 동기화로 인한 성능 문제를 피할 수 있음

        - 필드가 초기화된 이후로는 동기화하지 않기에 `volatile` 선언함

        - `result` 지역변수를 선언하여, 필드가 이미 초기화된 상황에 해당 필드를 1회만 읽도록 만듦
        ...
        - **변형**
            ...
            - **단일검사(single-check) 관용구**
                ...
                - 반복 초기화해도 되는 필드의 경우 사용하는 기법

            - 짜릿한 단일검사(racy single-check) 관용구

                - 반복 초기화해도 되고 필드의 타입이 `long`, `double`이 아닌 기본 타입인 경우 사용
                - 단일검사 관용구에서 `volatile` 키워드를 없앤 방식
                - 일반적으로는 사용하지 않음
...
- 스레드 스케줄러
    - 여러 스레드가 실행중일때, 스레드의 실행 시간을 결정
    - 스케줄링 정책은 운영체제에 따라 상이할 수 있음
    - 운영체제의 스레드 스케줄링 정책에 좌지우지되지 않는 프로그램을 만들어야 함
        그렇지 않을 경우 이식성이 매우 낮음
- 스케줄링 정책에 의존적이지 않게 설계하는 법
    - 실행 가능한 스레드의 평균 수를 프로세서 수보다 과도하게 많이 설정해선 안 됨
        - `실행 가능한 스레드 != 전체 스레드 수`임에 주의
    - 실행 준비가 된 스레드들이 맡은 작업을 완료할 때까지 실행하게 만듦
    - 스레드가 즉시 처리할 작업이 없을 경우 실행되지 않게 만듦
    - 스레드가 바쁜 대기(busy waiting) 상태가 되지 않게 만듦
        : 공유 객체의 상태를 끊임없이 검사하지 말자!
        - 스레드 스케줄러 영향이 높아지며, 프로세서에 큰 부담을 준다.
    - 특정 스레드가 CPU 시간을 얻지 못할 때 `Thread.yield()`로 해결하려고 하지 말자!
        - 이식성을 매우 낮추며, 다른 JVM에서는 성능이 떨어질 수 있으며, 테스트하기도 어려움
        - 애플리케이션 구조를 바꿔 동시 실행 가능한 스레드 수를 줄이는 편이 나음
    - 스레드 우선순위 조정의 경우에도 사용을 자제하자!
        - 우선순위 조정은 (특히 자바에서) 이식성이 매우 나쁨
        - 진짜 원인을 찾아 제거하는 편이 낫다.



# 12장 직렬화

객체 직렬화: 직렬화 + 역직렬화 메커니즘
...
- 직렬화의 도입
    - 1997년에 자바에 직렬화 도입되었음
    - 기존에는 연구용 언어인 모듈라-3(Modula-3)에서 시도되었으나,
        대중 언어에 적용된 적은 없었음
    - 분산 객체를 쉽게 만들 수 있다는 강점을 가지지만
        위험성에 대한 우려가 있었음
    - 현재까지는 우려되는 부분이 더 큰 것으로 여겨짐:
        보이지 않는 생성자, API와 구현 사이의 모호해진 경계, 잠재적인 정확성 문제, 성능, 보안, 유지보수성 등 ...
    - 샌프란시스코 시영 교통국(SFMTA; San Francisco Municipal Transportaion Agency)이 랜섬웨어 공격을 받아 마비된 사례도 있음



### 직렬화의 위험성

- ***직렬화는 공격 범위가 넓고, 지속적으로 더 넓어져 방어가 어렵다는 근본적인 문제를 갖는다.***

    - `ObjectInputStream.readObject()`의 호출은 `implements Serializable`인 거의 모든 객체를 만들어낼 수 있기 때문이다.
    - 자바 표준 라이브러리, 서드 파티 라이브러리, 애플리케이션 자신의 클래스 모두 공격 대상이 될 수 있음
    - 역직렬화는 애플리케이션에서 직접 사용될 뿐만 아니라, 자바 하부 시스템(RMI; Remote Method Invocation), JMX(Java Management Extension), JMS(Java Messaging System) 등에서 간접적으로 사용되고 있다. -> 잘못한 게 없는 애플리케이션이라도 문제가 발생할 수 있음
    - 원격 코드 실행 , DoS 공격 등으로 이어질 수 있다.

- 가젯(gadget)

    - 역직렬화 과정에서 호출되어 잠재적으로 위험한 동작을 수행하는 메서드
    - 가젯들을 함께 사용하여(가젯 체인) 공격자가 네이티브 코드까지도 마음대로 호출할 수 있게 되는 경우도 있다.

- 역직렬화 폭탄(deserialization bomb)

    - 역직렬화에 시간이 오래 걸리는 짧은 스트림, 이를 이용하여 DoS 공격 시도 가능
    ...
    - 예시
        ...
        static byte[] bomb {
            ...
            return serialize(root);		//해당 메서드의 코드는 생략
        }
        ```

        - `HashSet` 역직렬화 시에는 원소들의 해시코드도 계산해야 한다는 특징 때문에 직렬화가 사실상 끝나지 않음

> **직렬화 위험을 피하는 가장 좋은 방법은 아무 것도 직렬화하지 않는 것이다.**



### 대안: 크로스-플랫폼 구조화된 데이터 표현

- cross-platform structured-data representation
- 자바 직렬화보다 훨씬 나은 대안
- 대표적 예시
    - JSON: 텍스트 기반이라 사람이 읽을 수 있음
    - protobuf; Protocol Buffers: 이진 표현이기에 효율적
...
- 객체 역직렬화 필터링(`java.io.ObjectInputFilter`) 사용
    - 특정 클래스만 역직렬화하게 받아들이거나, 거부할 수 있음
    - 특정 클래스만 받아들이는 화이트리스트 방식이 안전
        SWAT; Serial Whitelist Application Trainer 도구가 화이트리스트를 생성해주는 것을 도움
    - 메모리를 과하게 사용하거나 객체 그래프가 과하게 깊어지는 것은 방지 가능하지만, 역직렬화 폭탄은 방지 불가



## 아이템86. `Serializable`을 구현할지는 신중히 결정하라

### `Serializable` 구현 시의 문제점들

- **문제점1: 한 번 `implements Serializable`로 릴리즈한 후에는 수정이 어려움**
    - 이 경우 직렬화된 바이트 스트림 인코딩도 일종의 공개 API가 되어 계속 지원해야 함
    
    - 자바 기본 직렬화 방식을 적용할 경우 `private`인 내부 필드를 외부로 공개하게 됨
    - 클래스 내부 개선을 시도하다가 직렬 버전 UID가 변경되어 버릴 수 있음
    
- **문제점2: 버그 및 보안 구멍의 가능성이 높아짐**
    - 직렬화를 추가함으로써 생성자를 우회한 객체 생성 방식을 하나 추가하는 것이기에,
        클래스 불변식이 깨질 빌미 제공함
    
- **문제점3: 신버전 릴리즈 시에 테스트할 사항이 증가**



### 주의사항

- **특수하게 `Serializable`을 구현해야 할 케이스**

    - 객체 전송 및 저장에 자바 직렬화를 이용하는 프레임워크를 위해 만들어진 클래스
    - `Serializable`을 구현해야 하는 클래스의 컴포넌트로 사용될 경우
    - 역사적으로는 값 클래스(`BigInteger`, `Instant`)나 컬렉션 클래스들의 경우는 구현,
        스레드 풀처럼 동작하는 객체를 표현하는 클래스의 경우 구현하지 않음

- **상속용으로 설계된 클래스 / 인터페이스의 경우 `Serializable` 구현하지 않아야 함**

    - 해당 클래스를 확장하거나 구현하는 측에 큰 부담이 되기 때문
    - 예외 케이스: `Throwable`, `Component`
        - `Throwable`의 경우 RMI를 통해 클라이언트로 예외를 보내기 위함
        - `Component`의 경우 GUI 전송, 저장, 복원을 위함

- **직렬화 / 확장이 모두 가능한 케이스의 경우 주의사항**

    - 불변식을 보장해야 하는 필드의 경우 `finalize` 재정의하지 못하게 만들어야 함

        - 해당 메서드를 `final`로 재정의하면 됨
        - `finalizer` 공격을 방지하기 위함

    - 기본값(0, null 등)으로 초기화될 경우 불변식이 깨지게 된다면, `readObjectNoData()` 추가해줘야 함
...
- **`Serializable` 구현하지 않을 경우 주의사항**

    - 하위 클래스에서 직렬화를 사용하려 할 때 부담을 고려하여 매개변수 없는 생성자를 제공할 수 있음
    - 그렇지 않을 경우 하위 클래스가 직렬화 프록시 사용해야 함

- **내부 클래스에는 직렬화를 구현해선 안 됨**

    - 바깥 인스턴스에 대한 참조 때문임
    - 정적 멤버 클래스의 경우 직렬화 구현해도 괜찮음



## 아이템87. 커스텀 직렬화 형태를 고려해보라

- `Serializable` 구현하고 기본 직렬화 형태를 사용할 경우 잘못된 API 설계했을때와 같은 문제에 시달릴 수 있다.

- **유연성, 성능, 정확성 측면에서 괜찮다고 판단될 때만 기본 직렬화 형태를 사용하라.**

- **객체의 물리적 형태와 논리적 내용이 같다면 기본 직렬화 형태도 무방하다.**
    ...
    - 위의 경우는 인스턴스 필드가 클래스의 논리적 구조를 잘 반영하고 있다.
    - `private` 필드임에도 문서화 주석이 달려 있다. 직렬화 형태에 포함되는 공개 API이기 때문

- **기본 직렬화 형태가 적합하더라도 불변식 보장과 보안을 위해 `readObject()` 메서드 제공해야 할 때가 많다.**

- **객체의 물리적 표현과 논리적 표현의 차이가 크다면 기본 직렬화 형태를 사용할 때 문제 발생한다.**

    - 공개 API가 현재의 내부 표현 방식에 영구히 묶임
    - 너무 많은 공간을 차지할 수 있음
    - 시간이 너무 많이 걸릴 수 있음
    - 스택 오버플로우 발생 가능성: 객체 그래프를 재귀 순회하기 때문
...
- 커스텀 직렬화 예시
    ...
    - 모든 필드가 `transient`임에도 `defaultWriteObject`, `defaultReadObject` 호출한다:
        나중에 `transient`가 아닌 인스턴스 필드가 추가되었을 경우에도 호환성을 갖추기 위함이다.

- **해당 객체의 논리적 상태와 무관한 필드라고 *확신할* 때만 `transient` 한정자를 생략하라.**

- **`transient` 필드들은 기본 직렬화 이용한 역직렬화시 기본값으로 초기화됨에 주의하라.**

    - `defaultReadObject` 호출 후 초기화하거나, 값을 처음 사용할 때 초기화하면 된다.

- **동기화 적용된 경우 동기화 매커니즘은 직렬화에도 적용되어야 한다.**

- **직렬화 가능 모든 클래스에는 직렬 버전 UID를 명시적으로 부여해야 한다.**

    ```java
    private static final long serialVersionUID = <무작위long값>;
    ```

    - 직렬 버전  UID로 인한 호환성 문제 방지
    - UID 생성으로 인한 성능 저하 문제 방지
    - 구버전과 호환성을 맞추고 싶을 경우 UID 일치, 아닐 경우 다른 UID 사용하기만 하면 된다.
        구버전과 호환성이 달라지는 것이 아닐 경우 UID는 절대 바꾸지 말자.



## 아이템88. `readObject` 메서드는 방어적으로 작성하라

- **`readObject()`는 실질적으로 `public` 생성자다.**

    - 인수 유효성 검사 필요하며, 경우에 따라 매개변수를 방어적 복사해야 한다.
    - 매개변수로 바이트스트림을 받는 생성자로 이해하자.
    - 생성자와 마찬가지로 재정의 가능 메서드를 호출해선 안 된다.

- **`defaultReadObject()` 호출 후에는 역직렬화된 객체의 유효성을 검사하라.**

- **방어적 복사를 통해 `private` 가변 요소의 수정을 막아라.**
...
- 안전한 `readObject()` 예시
    ...
    - 방어적 복사를 유효성 검사보다 우선 수행함
    - `clone()` 사용하지 않았음
    - `final` 필드는 방어적 복사가 불가하기에  `final` 제거
...
- 직렬화에 의해 깨질 수 있는 싱글톤
    ...
    - 직렬화 가능한 객체가 되면(`implements Serializable`) 싱글톤이 깨짐
    - `readObject()` 별도 제공하더라도 깨지는 것 막을 수 없음

- `readResolve` 이용한 해결

    - `readObject()`가 만들어낸 인스턴스를 교체할 수 있음, 이를 이용해 기존 인스턴스 참조 연결하고, 새로 생성된 인스턴스는 GC 대상으로 만들 수 있음
    ...
    - 위 경우 역직렬화 데이터가 필드를 가질 필요가 없기에 모든 필드가 `transient`로 선언되어야 함
        (객체 참조 타입에 대해서, 인스턴스 통제 목적으로 `readResolve()` 사용 시 공통적인 사항)
    - `transient` 선언 안 할 경우 역직렬화된(GC 대상이 될) 객체에 대한 참조 공격이 가능하기 때문

- 대안: 원소 1개짜리 `enum`으로 싱글톤  만들기

    - `readResolve()` 이용한 방식은 복잡하지만 `enum` 이용한 방식은 간단
    - 단, 컴파일 시점에 어떤 인스턴스가 있는지 알 수 없다면 `readResolve()` 사용할 수밖에 없음

- `readResolve()` 메서드의 접근 제한자 설정
    - `final` 클래스의 경우 `private`
    - 이 외의 경우 가능한 한 접근 최소화하는 편이 좋음
...
- 직렬화 프록시 패턴(serialization proxy pattern)
    ...
    - 구현 방식
        ...
        - 직렬화 프록시
            - 외부 클래스의논리적 상태를 표현하는 `private static` 중첩 클래스
            - 외부 클래스를 매개변수로 받는 하나의 생성자만 가져야 함: 복사 수행
    ...
    - 예시
        ...
        private static class SerializationProxy implements Serializable {
            ...
            private static final long serialVersionUID = <무작위 수>;
        }
        ```
        ...
        public class Period implements Serializable {
            ...
            private void readObject(ObjectInputStream stream) 
                ...
                throw new InvalidObjectException("프록시가 필요함");
            }
            ...
        }
        ````
        ...
        - `writeReplace()`메서드
            - 자바 직렬화 시스템이 직렬화 프록시를 반환하게 만듦
            - 이로 인해 `Period` 자체의 직렬화된 인스턴스가 생성되지는 않음
        - `readObject()` 메서드를 이용해 불변성 훼손 시도를 막을 수 있음
        - 직렬화 프록시의 `readResolve()` 이용하여 바깥 인스턴스를 반환하게 만듦

    - 강점

        - `final` 선언 가능하기에 불변 클래스 가능
        - 생성자 통한 객체 생성이 가능하기에 직렬화로 인한 문제 방어 가능
        - 역직렬화된 인스턴스와 직려로하된 인스턴스 클래스가 달라도 작동

    - 한계

        - 클라이언트가 마음대로 확장 가능한 클래스에는 적용 불가
        - 객체 그래프에 순환 있을 경우 사용 불가
        - 성능 하락
